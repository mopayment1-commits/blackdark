...
def _parse_env_file(path: Path) -> dict[str, str]:
    ...
    try:
        ...
        # Do not log path names for secret files (CodeQL clear-text log).
        logger.warning("secrets_file_unreadable")
        return out
...
def load_secrets_file(env_pointer: str, *, override: bool = False) -> bool:
    ...
    if not path.is_absolute():
        path = _ROOT / path
    if not path.is_file():
        logger.warning("secrets_file_missing")
        return False
    for key, value in _parse_env_file(path).items():
        if override or not os.getenv(key, "").strip():
            os.environ[key] = value
    _LOADED.add(pointer)
    logger.info("secrets_file_loaded keys=present")
    return True
    ...
    if not os.getenv("TELEGRAM_BOT_TOKEN", "").strip() and _DEFAULT_TELEGRAM_SECRETS.is_file():
        ...
        for key, value in _parse_env_file(_DEFAULT_TELEGRAM_SECRETS).items():
            if not os.getenv(key, "").strip():
                os.environ[key] = value
        _LOADED.add(pointer)
        logger.info("secrets_file_loaded keys=present source=default")
