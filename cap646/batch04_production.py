...
def _stamp_batch04(result: dict[str, Any], capability_id: int) -> dict[str, Any]:
    ...
    result["production_spine"] = "batch04_prep"
    nested = result.get("data") if isinstance(result.get("data"), dict) else {}
    if not result.get("data_source") and not result.get("source"):
        result["data_source"] = (
            result.get("data_source") or result.get("source") or nested.get("data_source")
            or nested.get("source") or f"cap646.batch04_production#cap{capability_id:03d}"
        )
...
def _make_cap_entrypoint(capability_id: int) -> Callable[..., Awaitable[dict[str, Any]]]:
    ...
    _entry.__doc__ = f"Batch04 prep production entrypoint for capability #{capability_id}."
    return _entry
