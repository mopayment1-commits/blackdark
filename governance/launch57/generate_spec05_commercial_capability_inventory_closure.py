#!/usr/bin/env python3
"""Generate SPEC_05 Commercial Capability Inventory closure artifacts."""
...
OUT = ROOT / "governance" / "launch57" / "SPEC_05_COMMERCIAL_CAPABILITY_INVENTORY"


def _md_truth_table(rows: list[dict]) -> str:
    lines = [
        "# SPEC_05 Runtime Truth Table",
        "",
...
def _md_local_closure(status: dict, iv: dict, tests: dict) -> str:
    ...
    if not gap_lines:
        gap_lines = "- None"
    return f"""# SPEC_05 Local Closure Report
...
- `inventory_count_valid`: {status.get('inventory_count_valid')}
...
def main() -> None:
    ...
    requirements = {
        "artifact": "SPEC_05_REQUIREMENTS_REGISTER",
