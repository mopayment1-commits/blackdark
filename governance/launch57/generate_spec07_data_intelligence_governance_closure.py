#!/usr/bin/env python3
"""Generate SPEC_07 Data Intelligence Governance closure artifacts."""
...
OUT = ROOT / "governance" / "launch57" / "SPEC_07_DATA_INTELLIGENCE_GOVERNANCE"


def _md_truth_table(rows: list[dict]) -> str:
    lines = [
        "# SPEC_07 Runtime Truth Table",
        "",
...
def _md_local_closure(status: dict, iv: dict, tests: dict) -> str:
    ...
    if not gap_lines:
        gap_lines = "- None"
    return f"""# SPEC_07 Local Closure Report
...
- `runtime_enforcement_ok`: {status.get('runtime_enforcement_ok')}
...
def main() -> None:
    ...
    requirements = {
        "artifact": "SPEC_07_REQUIREMENTS_REGISTER",
