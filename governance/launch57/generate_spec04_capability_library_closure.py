#!/usr/bin/env python3
"""Generate SPEC_04 Capability Library closure artifacts."""
...
OUT = ROOT / "governance" / "launch57" / "SPEC_04_CAPABILITY_LIBRARY"


def _md_truth_table(rows: list[dict]) -> str:
    lines = [
        "# SPEC_04 Runtime Truth Table",
        "",
...
def _md_local_closure(status: dict, iv: dict, tests: dict) -> str:
    ...
    if not gap_lines:
        gap_lines = "- None"
    return f"""# SPEC_04 Local Closure Report
...
- `library_scope_ok`: {status.get('library_scope_ok')}
- `entitlement_align_ok`: {status.get('entitlement_align_ok')}
...
def main() -> None:
    ...
    requirements = {
        "artifact": "SPEC_04_REQUIREMENTS_REGISTER",
