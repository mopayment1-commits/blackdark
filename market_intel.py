...
async def _fetch_futures_market_context(
    ...
    try:
        ...
        async with session.get(funding_url) as f_resp:
            funding = await f_resp.json() if f_resp.status == 200 else {}
    except (aiohttp.ClientError, TypeError, ValueError):
        return None
    return oi_data, ticker, funding


def _open_interest_signal(oi_usd: float, change: float, funding_rate: float) -> str:
...
def _open_interest_row(
    ...
    return {
        ...
        "signal": _open_interest_signal(oi_usd, change, funding_rate),
    }
...
async def fetch_open_interest(symbols: list[str] | None = None) -> list[dict[str, Any]]:
    ...
    async with aiohttp.ClientSession(timeout=timeout) as session:
        for asset in assets:
            ...
            if not pair.isalnum():
                continue
            context = await _fetch_futures_market_context(session, pair)
            if context is None:
                continue
            results.append(_open_interest_row(asset, *context))
...
def _ensure_gravity_node(nodes: dict[str, dict[str, Any]], asset: str, sector: str) -> dict[str, Any]:
...
def _apply_whale_alert(
    ...
    *,
    ...
    node = _ensure_gravity_node(
        nodes,
        asset,
        alert.get("sector") or config.SECTOR_MAP.get(asset, "Other"),
    )
    node["gravity_score"] += min(50, score * 0.5)
    node["alert_count"] += 1
    node["net_flow_usd"] += float(meta.get("net_flow_usd") or 0)


def _apply_sector_flow(
    nodes: dict[str, dict[str, Any]],
    flow: dict[str, Any],
    price_by_asset: dict[str, dict[str, Any]],
    *,
    parse_metadata,
) -> None:
    ...
    for asset, pdata in price_by_asset.items():
        ...
        if asset_sector != sector and pdata.get("sector") != sector:
            continue
        _ensure_gravity_node(nodes, asset, asset_sector)["gravity_score"] += sii * 0.12


def _gravity_label(gravity: float) -> str:
    if gravity >= 55:
        return "High gravity"
    if gravity >= 28:
        return "Moderate"
...
def _gravity_bubble(node: dict[str, Any], market: dict[str, Any]) -> dict[str, Any]:
...
def build_whale_gravity_map(
    ...
    *,
    ...
    for alert in whale_ctx.get("whale_alerts") or []:
        _apply_whale_alert(nodes, alert, parse_metadata=parse_metadata)

    for flow in whale_ctx.get("sector_flows") or []:
        _apply_sector_flow(nodes, flow, price_by_asset, parse_metadata=parse_metadata)

    bubbles = []
    for asset, node in nodes.items():
        bubbles.append(_gravity_bubble(node, price_by_asset.get(asset, {})))
