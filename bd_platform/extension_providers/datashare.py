"""Datashare provider contract (#648)."""
...
PROVIDER_NAME = "datashare"
REQUIRED_CONFIG_KEYS = ("provider_name", "warehouse_id")
...
def health_status(*, config: dict[str, Any] | None = None, external_reason: str = "") -> ProviderStatus:
    ...
    ready = bool(config.get("enabled")) and valid and bool(config.get("credential_ref")) and bool(config.get("warehouse_id"))
    return ProviderStatus(
        ...
        reason=external_reason or (";".join(errors) if errors else "warehouse_agreement_required"),
        provenance={"config_valid": valid, "warehouse_id": config.get("warehouse_id")},
    )
...
def execute_local_contract(*, symbol: str = "BTC", config: dict[str, Any] | None = None, external_reason: str = "") -> dict[str, Any]:
    ...
    if status.ready:
        return {
            ...
            "warehouse_id": (config or default_config()).get("warehouse_id"),
            "analysis_only": True,
            "no_execution": True,
        }
