...
_MANUAL: dict[int, tuple[str, str]] = {
    ...
    642: ("cap646.batch01_production", "execute"),
    644: ("cap646.batch01_production", "execute"),
    645: ("bd_platform.batch13_operational_intelligence_layer", "security_verification_evidence_645"),
    646: ("cap646.handlers.institutional", "handle_institutional_capability"),
...
def batch_test_module_for(capability_id: int) -> str | None:
    ...
    if batch04.is_file():
        ...
        if capability_id in batch04_ids:
            return "tests/test_charting_market_intelligence_batch301_400.py"
    batch05 = ROOT / "scripts" / "partial_batches" / "batch_05_401_500.json"
    if batch05.is_file():
        import json
