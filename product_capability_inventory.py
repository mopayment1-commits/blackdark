...
def capability_catalog() -> list[dict[str, Any]]:
    return [
        ...
        _f(id="ID-OAUTH", name="OAuth start/callback", name_ar="بدء/رجوع OAuth", domain="identity", status="ops_config", personas=ALL, surfaces=["/api/auth/oauth/{provider}/start"], evidence="api/routers/auth.py", efficiency="Code path present; live IdP needs client ids", unpaid_block="oauth_client_ids"),
        _f(id="ID-EMAIL", name="Email verify / password reset", name_ar="تحقق البريد / إعادة كلمة المرور", domain="identity", status="ops_config", personas=ALL, surfaces=["/verify-email", "/reset-password"], evidence="dashboard.py + mail transport", efficiency="Pages render; production mail is ops", unpaid_block="mail_transport"),
        ...
        _f(id="BIL-CHECKOUT", name="Self-serve checkout Stripe/Lemon", name_ar="دفع ذاتي Stripe/Lemon", domain="billing", status="partial", personas=["pro", "whale"], surfaces=["/api/billing/checkout", "/create-checkout-session", "/webhook"], evidence="api/routers/billing.py", efficiency="Code ready; live charge needs PSP secrets", unpaid_block="psp_credentials"),
        ...
        _f(id="OR-E2E", name="Decision e2e LIVE→RISK→DECISION", name_ar="قرار طرف-لطرف حي→مخاطرة→قرار", domain="oracle", status="partial", personas=DESK, surfaces=["/api/institutional/decision-e2e"], evidence="oms_decision.py", efficiency="Live mids; learning_self_grade=false calibration stub"),
        ...
        _f(id="MKT-L2", name="Institutional venue L2 books", name_ar="كتب L2 مؤسسية حسب المنصة", domain="market", status="partial", personas=DESK, surfaces=["live_data_truth_probe", "native_regional_cex_fetcher"], evidence="80/100 venue_l2; 20 synthetic_mid", efficiency="Native CEX L2 unpaid max; AMM+bybit remain", unpaid_block="amm_and_bybit_geo"),
        ...
        _f(id="MKT-SENT", name="Sentiment / onchain / macro overviews", name_ar="نظرة معنويات / سلسلة / كلي", domain="market", status="partial", personas=PAID, surfaces=["/api/sentiment/overview", "/api/onchain/overview", "/api/macro/overview"], evidence="dashboard.py", efficiency="Free-tier public proxies; not Kaiko/Glassnode exclusive"),
        _f(id="MKT-OPT", name="Options overview", name_ar="نظرة الخيارات", domain="market", status="partial", personas=["pro", "whale"], surfaces=["/api/options/overview"], evidence="dashboard.py", efficiency="Overview only; not a full options OMS"),
        ...
        _f(id="ARB-CAT", name="Arbitrage type catalog", name_ar="كتالوج أنواع الأربتراج", domain="arbitrage", status="partial", personas=["pro", "whale"], surfaces=["/api/arbitrage/catalog"], evidence="api/routers/arbitrage.py", efficiency="Honest live/proxy/planned mix"),
        _f(id="ARB-CEXDEX", name="CEX↔DEX path", name_ar="مسار منصة↔DEX", domain="arbitrage", status="partial", personas=["whale"], surfaces=["/api/platform/arb/cex-dex", "/api/execution/cex-dex/cycle"], evidence="platform_api.py", efficiency="Indicative≠executable without live fill"),
        # ── Execution ─────────────────────────────────────────
        ...
        _f(id="EX-JUP", name="Jupiter quote/build/local sign", name_ar="Jupiter تسعير/بناء/توقيع محلي", domain="execution", status="partial", personas=["whale"], surfaces=["/api/institutional/jupiter/*"], evidence="signed_local=true; RPC VC blocked", efficiency="Local sign OK; unfunded wallet", unpaid_block="wallet_unfunded_zero_cost_constraint"),
        ...
        _f(id="AL-SUB", name="Alert subscribe Telegram/email", name_ar="اشتراك تنبيهات تيليغرام/بريد", domain="alerts", status="partial", personas=["pro", "whale"], surfaces=["/api/alerts/subscribe", "/api/alerts/telegram/*", "/api/telegram/*"], evidence="alert_service.py telegram.py", efficiency="Path present; live channel needs bot token", unpaid_block="telegram_bot_token"),
        ...
        # ── ML / platform extras ──────────────────────────────
        _f(id="ML-TRAIN", name="ML train/predict/ensemble/experience", name_ar="تدريب/تنبؤ/تجميع/خبرة تعلم آلي", domain="ml", status="partial", personas=["pro", "whale", "acquirer"], surfaces=["/api/ml/*", "/api/platform/ml/*"], evidence="platform_api.py", efficiency="Local models; not a hosted GPU farm"),
        _f(id="ML-EXPLAIN", name="ML explain / TruLens fallback", name_ar="تفسير النموذج / TruLens احتياطي", domain="ml", status="partial", personas=["pro", "whale"], surfaces=["/api/platform/ml/explain"], evidence="platform_api.py", efficiency="Optional TruLens"),
        _f(id="PLAT-GRID", name="Grid bots / rules / marketplace / calendars", name_ar="بوتات شبكة / قواعد / سوق استراتيجيات / تقاويم", domain="platform", status="partial", personas=["pro", "whale"], surfaces=["/api/platform/bots/grid", "/api/platform/rules", "/api/platform/marketplace/strategies", "/api/platform/events/calendar", "/platform"], evidence="platform_api.py", efficiency="In-process toys + public proxies; not Kaiko-class"),
        _f(id="PLAT-DERIV", name="Derivatives hub / liquidations / CEX-DEX compare", name_ar="مركز مشتقات / تصفيات / مقارنة منصة-DEX", domain="platform", status="partial", personas=["pro", "whale"], surfaces=["/api/platform/derivatives/*", "/api/platform/liquidations/radar"], evidence="platform_api.py", efficiency="Public proxies"),
        _f(id="PLAT-TV", name="TradingView config / webhook", name_ar="إعداد TradingView / ويب هوك", domain="platform", status="partial", personas=["pro", "whale"], surfaces=["/api/platform/tradingview/*"], evidence="platform_api.py", efficiency="Webhook path; live TV is ops"),
        # ── B2B / WL / org ────────────────────────────────────
        ...
        _f(id="B2B-WL", name="White-label brand/portal/terminal/exports", name_ar="علامة بيضاء علامة/بوابة/طرفية/تصدير", domain="b2b", status="partial", personas=["b2b", "fund"], surfaces=["/api/institutional/orgs/{id}/portal", "/api/institutional/white-label/*"], evidence="white_label.py", efficiency="In-process; hosted_custom_domain=false", unpaid_block="hosted_custom_domain_requires_paid_infra"),
        ...
        _f(id="B2B-SSO", name="OIDC/SAML SSO", name_ar="تسجيل دخول موحد OIDC/SAML", domain="b2b", status="partial", personas=INST, surfaces=["/api/institutional/sso/*"], evidence="enterprise_sso.py", efficiency="Crypto path; live IdP is ops", unpaid_block="idp_credentials"),
        _f(id="B2B-SCIM", name="SCIM 2.0 Users/Groups", name_ar="SCIM 2.0 مستخدمون/مجموعات", domain="b2b", status="partial", personas=INST, surfaces=["/api/institutional/scim/*"], evidence="api/routers/institutional.py", efficiency="CRUD when SCIM_BEARER_TOKEN set", unpaid_block="scim_bearer_token"),
        ...
        _f(id="FUND-IR", name="IR / backup drill / dump-restore / recovery bundle", name_ar="استجابة حوادث / نسخ / استعادة / حزمة تعافٍ", domain="ops", status="partial", personas=["acquirer"], surfaces=["/api/institutional/ops/recovery", "/api/institutional/ops/recovery-bundle", "/api/database/health"], evidence="oms_decision.py", efficiency="In-process drills; not paid multi-AZ"),
        ...
        _f(id="DD-LAUNCH", name="Launch checklist / production guard / GTM", name_ar="قائمة إطلاق / حارس إنتاج / GTM", domain="dd", status="partial", personas=["acquirer"], surfaces=["/admin/launch", "/api/launch/readiness", "/api/gtm/status"], evidence="launch_checklist.py production_guard.py", efficiency="Domain/golive may remain pending ops"),
        ...
        _f(id="SITE-GQL", name="GraphQL router", name_ar="موجّه GraphQL", domain="site", status="partial", personas=PAID, surfaces=["create_graphql_router"], evidence="dashboard include_router", efficiency="Present; not the primary contract"),
        ...
        _f(id="INV-FULL", name="Full capability inventory API (this catalog)", name_ar="واجهة جرد القدرات الكاملة", domain="dd", status="works", personas=ALL, surfaces=["/api/product/capability-inventory"], evidence="product_capability_inventory.py", efficiency="Binding machine-readable inventory; NOT_COMPLETE"),
    ]
    ...
    return {
        ...
        "ask_3_efficiency": {
            "unpaid_paths_verified_by": [
                "import smoke of inventory + persona matrix",
                "pytest tests/test_full_capability_inventory.py",
                "GET /api/product/capability-inventory product_complete=false",
                "CORE mesh 77/77 and catalog venue_l2 80/100 (prior prove)",
                "/settings/security → 307 /profile",
            ],
            "not_claimed": [
                "live_fill",
                "jupiter_onchain_vc",
        ...
        "ask_4_nothing_forgotten_or_broken": {
            ...
            "fixed_this_wave": [
                "full capability inventory API",
                "institutional 8-ask review document",
            ],
        ...
        "ask_6_defects_and_weaknesses": [
            ...
            {"id": "telegram_token", "type": "ops", "impact": "Pro alerts may stay simulated"},
            {"id": "e2e_calibration_stub", "type": "product_partial", "impact": "learning_self_grade=false until later market window"},
            {"id": "wl_not_hosted_saas", "type": "unpaid_ceiling", "impact": "B2B is in-process portal, not custom-domain SaaS"},
            {"id": "sentiment_proxies", "type": "product_partial", "impact": "Not exclusive paid market-data vendors"},
            {"id": "oauth_mail_ops", "type": "ops", "impact": "Social login / mail verify need owner secrets"},
        ],
        ...
        "ask_8_must_still_implement": {
            "unpaid_remaining": [
                "Independent time-separated decision outcome instead of e2e calibration_stub (needs later market window)",
                "Do not invent AMM CEX-style L2 — keep synthetic_mid labeled",
            ],
            "ops_owner": [
                "PSP secrets",
                "Telegram bot token",
