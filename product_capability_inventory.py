...
def capability_catalog() -> list[dict[str, Any]]:
    ...
    return [
        ...
        _f(id="OR-IDK", name="I DON'T KNOW public epistemic token", name_ar="توكن علني: لا أعرف", domain="oracle", status="works", personas=ALL, surfaces=["/oracle/{symbol}", "/api/regulatory/compliance"], evidence="epistemic_honesty.py regulatory_compliance_guard.py", efficiency="Conflict/insufficient evidence publish I_DONT_KNOW; not a directional WAIT"),
        _f(id="OR-MIND", name="Public Changed-Mind Record", name_ar="سجل تغيّر الرأي العلني", domain="oracle", status="works", personas=ALL, surfaces=["/changed-mind", "/api/public/changed-mind"], evidence="changed_mind_record.py", efficiency="Derived from sealed audit-chain view revisions; empty chain is published honestly"),
        ...
        _f(id="MKT-L2", name="Institutional venue L2 books", name_ar="كتب L2 مؤسسية حسب المنصة", domain="market", status="partial", personas=DESK, surfaces=["live_data_truth_probe", "/api/product/l2-remainder"], evidence="95/100 venue_l2; remainder labeled synthetic_mid", efficiency="Wave 7 native L2 unpaid max; remaining AMM+bybit not invented as CEX ladders", unpaid_block="amm_and_bybit_geo"),
        _f(id="MKT-MESH", name="CORE public CEX L2 mesh", name_ar="شبكة L2 للمنصات العامة الأساسية", domain="market", status="works", personas=DESK, surfaces=["prove_multi_venue_live", "/api/institutional/canonical/mesh-prove"], evidence="92/92 live L2", efficiency="CORE mesh complete for this wave; catalog L2 is separate"),
    ...
    return {
        ...
        "ask_3_efficiency": {
            "unpaid_paths_verified_by": [
                ...
                "CORE mesh 92/92 and catalog venue_l2 95/100",
            ],
            "not_claimed": [
                "live_fill",
                "jupiter_onchain_vc",
        ...
        "ask_8_must_still_implement": {
            "unpaid_remaining": [
                "Do not invent AMM CEX-style L2 — keep synthetic_mid labeled (95/100 is wave-7 unpaid L2, not 100%)",
            ],
            "ops_owner": [
                "PSP secrets",
                "Telegram bot token",
