...
_PLAN_ROWS: tuple[tuple[Any, ...], ...] = (
    ...
    ("core", "Fast price updates (WebSocket)", "complete", "exchange_ws_hub", "/api/low-latency/status", "WS hub + stream freshness truth (LIVE/STALE)"),
    ("core", "100 exchanges — phase 1", "partial", "universe_rollout", "/api/universe/rollout", "Catalog≥100; live health separate — not all live"),
    ("core", "Cross-exchange arbitrage", "complete", "arbitrage_engine", PATH_API_ARBITRAGE_OPPORTUNITIES, "cross_exchange live + canonical adoption"),
    ("core", "Triangular arbitrage", "complete", "arbitrage_engine", PATH_API_ARBITRAGE_OPPORTUNITIES, "triangular live + canonical adoption"),
    ("core", "Funding rate harvest", "complete", "arbitrage_engine", PATH_API_ARBITRAGE_OPPORTUNITIES, "funding depth fail-closed + canonical"),
    ("core", "Spot vs Futures", "complete", "arbitrage_engine", PATH_API_ARBITRAGE_OPPORTUNITIES, "spot_futures live + canonical adoption"),
    ("core", "CEX ↔ DEX", "partial", "bd_platform.cex_dex_executor", "/api/platform/arb/cex-dex/execute", "L2+fee+gas gates; executable only when CEX book walk succeeds"),
    ...
    ("features", "Execution Speed", "complete", "fast_scan_engine", "/api/low-latency/fast-scan", "millisecond warm path"),
    ...
    ("ai", "Whale Intelligence", "partial", "whale_tracker", "/api/whale-activity", "CVVD + whale_execution_evidence (measured readiness gated)"),
    ("ai", "Portfolio AI", "partial", "portfolio_intelligence", PATH_API_PORTFOLIO_ANALYZE, "concentration + scenarios; institutional depth ongoing"),
    ...
    ("inst", "Institutional B2B", "complete", "b2b_institutional_ops", "/b2b", "WS feed + reporting/alerts/SLA + white-label"),
    ("inst", "Research Lab", "complete", "research_lab", "/api/research/lab", "VaR/NVT/MVRV/SOPR proxies"),
    ("inst", "On-chain Intelligence", "complete", "onchain_tracker", "/api/onchain/overview", "flows matrix + risk integration"),
    ...
    ("mobile", "Web / Desktop / Mobile apps", "complete", "dashboard", "/", "PWA (manifest + service worker)"),
    ("data", "NLP Sentiment", "complete", "sentiment_engine", "/api/sentiment/overview", "Twitter/Reddit + VADER"),
    ("data", "SEC filings AI", "complete", "sec_filings_ai", "/api/research/sec-filings", "EDGAR Atom ingest + keyword risk classification"),
)
