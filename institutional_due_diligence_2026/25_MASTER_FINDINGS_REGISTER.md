...
| WF-027 | 0 | P2 | Dormant Cherry-Pick Legacy IDs Pending Future Batch Audits | **MONITORED — NO ACTIVE CONFLICT** |
...
**WF-027 (Run 009 — 2026-09-11):** Dormant legacy cherry-pick IDs `{175, 214, 245, 584, 629, 630, 631, 642, 644, 646}` remain in `LEGACY_BATCH01_EXTENSION_IDS` with **zero active routing overlap** vs BATCH02/BATCH03 (Run 008 Item 4 rg proof). Status: `MONITORED — NO ACTIVE CONFLICT`. **Mandatory activation trigger:** when opening any future batch (Batch04+) whose official ID range includes any of these ten IDs, the **first mandatory step** before any nine-phase phase is a CROSS-SPINE-001 re-check on that specific ID (Run 007/008 methodology). **Closure condition:** only after closure of the last batch containing any of these IDs (approx. Batch07 for 642–646). Do **not** build proactive handlers outside an open batch audit scope (audit precedes build — Run 003 discipline).
