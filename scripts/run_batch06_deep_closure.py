#!/usr/bin/env python3
"""Batch 06 (501-600) deep closure: live exec, quad audit, REUSED-LINK tagging."""
...
BATCH_MANIFEST = ROOT / "scripts/partial_batches/batch_06_501_600.json"
EVIDENCE = ROOT / "data/hero_batch_06_501_600_evidence.jsonl"
GAP = ROOT / "docs/HERO_BATCH_06_501_600_GAP_REPORT.json"
AUDIT_JSON = ROOT / "docs/RETROSPECTIVE_DEEP_AUDIT_BATCH_06_501_600.json"
AUDIT_MD = ROOT / "docs/RETROSPECTIVE_DEEP_AUDIT_BATCH_06_REPORT.md"
SAMPLE_DOSSIER = ROOT / "docs/HERO_BATCH_06_SAMPLE_DOSSIER.json"


async def audit_batch06() -> dict:
...
def write_md(report: dict) -> None:
    ...
    reused = [r for r in report["rows"] if r["classification"] == "REUSED-LINK"]
    lines = [
        "# Retrospective Deep Audit — Batch 06 (501–600)",
        "",
    ...
    for r in reused:
        meta = r.get("reuse_meta", {})
        reasons = meta.get("reuse_reasons", [])
        lines.append(
            f"- #{r['capability_id']}: `{r['underlying_module']}.{r['underlying_function']}` — {', '.join(reasons)}"
        )
    lines += [
        "",
        "## Gate",
        "",
        "- Batch 07 **blocked** until explicit user approval.",
        "",
...
def write_sample_dossier(report: dict) -> None:
    ...
    sample = random.Random(60600).sample(quad_pass, min(10, len(quad_pass)))
    dossier = {
        "batch": "batch_06_501_600",
        "sample_size": len(sample),
        "selection_seed": 60600,
        "capabilities": sample,
    }
...
async def main() -> int:
    ...
    if not args.skip_closure:
        print("Running batch 06 closure (live exec + gap report)...")
        summary = await run_closure("batch_06_501_600", upgrade=not args.skip_upgrade)
        print(json.dumps({k: summary[k] for k in ("live_ok", "live_fail", "implemented_native", "delegated", "deferred")}, indent=2))

    print("Running batch 06 deep quad audit...")
    report = await audit_batch06()
    ...
    _update_gap(GAP, report["rows"], id_set, "batch_06_501_600")
    _update_checklist(report["rows"])
    write_md(report)
    write_sample_dossier(report)

    c = report["classification_counts"]
    print("\n=== BATCH 06 HONEST COUNT ===")
