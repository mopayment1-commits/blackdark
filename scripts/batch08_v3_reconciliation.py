#!/usr/bin/env python3
"""Batch08 v3 reconciliation — state vocabulary, duplicate coverage, hero matrix, SSOT, forensics."""
...
BATCH08_IDS = recon.BATCH08_IDS
EXPECTED_COUNT = recon.EXPECTED_COUNT
PRIOR_IDS = list(range(1, 351))
EXPECTED_CROSS_BATCH_PAIRS = len(BATCH08_IDS) * len(PRIOR_IDS)
        ...
        rows.append(
            {
                ...
                "binding": f"{mod}.{fn}",
                "evidence": f"docs/docs/RETROSPECTIVE_DEEP_AUDIT_BATCH_04_301_400.json#capability_id={cid}",
            }
        )

    counts = Counter(r["canonical_state"] for r in rows)
    return {
        "artifact": "BATCH08_V3_STATE_CLASSIFICATION",
...
def _pair_decision(
    ...
    if batch08_id in recon.REUSED_LINK_CATALOG and prior_id == recon.REUSED_LINK_CATALOG[batch08_id]["canonical_capability_id"]:
        return {
...
def build_cross_batch_exhaustive_coverage(
                ...
                material_overlaps.append(
                    {
                        ...
                        "batch08_binding": f"{b_bind[0]}.{b_bind[1]}",
    ...
    return {
        "scope": "Exhaustive machine coverage — Batch08 351-400 vs prior 1-350",
        "method": (
            "Deterministic pair evaluator over all 50×300 candidate pairs. "
            "No indexing filter may omit pairs — each (batch08_id, prior_id) receives a decision. "
        ...
        "candidate_filter_false_negative_risk": (
            ...
            "Material duplicate detection uses binding equality, canonical reuse (#379→#189), and normalized name overlap."
        ),
        "overlap_dimensions_reviewed": [
            ...
            "route/API overlap (unique pdf_capability_registry bindings per batch08 ID)",
            ...
            "Hero contribution overlap (#379 facade CONTEXT only; no FEED double-count)",
        ],
...
def _hero_cell(cid: int, hero: dict[str, str], catalog_name: str, track: str) -> dict[str, Any]:
    ...
    if cid == 379 and hid == "H1":
        return {
            "relationship": "CONTEXT",
            "reason": "Facade delegates to canonical #189 liquidity engine — contextual reuse, not additive Hero FEED",
            "evidence_location": "docs/BATCH08_DUPLICATE_CANONICAL_ANALYSIS.json#379_to_189",
        }
    if cid == 382 and hid == "H5":
        return {
            "relationship": "CONTEXT",
            "reason": "Single-Sentence Oracle CTA facade — UI context only, not additive FEED",
            "evidence_location": "docs/BATCH08_SIX_HEROES_BINDING.json#capability_id=382",
        }
    if cid in (356, 390) and hid == "H2":
        return {
            "relationship": "CONTEXT",
            "reason": "Market/whale intelligence RSS and exchange-health surfaces — context labels only",
            "evidence_location": f"docs/BATCH08_HERO_MATRIX_351_400.json#capability_id={cid}#{hid}",
        }
    if cid in (369, 392) and hid == "H1":
        return {
            "relationship": "CONTEXT",
            "reason": "Cross-domain decision intelligence — opportunity framing context, not direct FEED",
            "evidence_location": f"docs/BATCH08_HERO_MATRIX_351_400.json#capability_id={cid}#{hid}",
        }
    if cid in (384, 385, 386, 387) and hid == "H5":
        return {
            "relationship": "CONTEXT",
            "reason": "AI agent / prompt-to-SQL charting — oracle-adjacent UI context only",
            "evidence_location": f"docs/BATCH08_HERO_MATRIX_351_400.json#capability_id={cid}#{hid}",
        }
    if cid in (366, 367, 389) and hid == "H3":
        return {
            "relationship": "CONTEXT",
            "reason": "Data methodology/provenance/lineage supports ledger integrity narrative — not a ledger FEED",
            "evidence_location": f"docs/BATCH08_HERO_MATRIX_351_400.json#capability_id={cid}#{hid}",
        }
...
def build_hero_matrix_50x6(
        ...
        for hero in CANONICAL_HEROES:
            ...
            if cell["relationship"] == "FEED":
                cell["feed_proof"] = {
                    ...
                    "double_count_review": "no duplicate Hero score — CONTEXT/NA default for batch08 charting",
                }
            cells.append(cell)
    ...
    if any(c["capability_id"] == 379 and c["relationship"] == "FEED" for c in cells):
        duplicate_contribs.append("379_feed")
    if any(c["capability_id"] == 382 and c["relationship"] == "FEED" for c in cells):
        duplicate_contribs.append("382_feed")
    ...
    return {
        "artifact": "BATCH08_HERO_MATRIX_351_400",
        "generated_at": datetime.now(UTC).isoformat(),
        "git_commit": baseline_head,
        ...
        "summary": {
            ...
            "379_no_double_count_with_189": "379" not in duplicate_contribs,
            "382_no_hero_double_count": "382" not in duplicate_contribs,
        },
        "cells": cells,
    }
...
async def _benchmark_full_path(cid: int) -> dict[str, Any]:
    ...
    user = {"id": 1, "email": "batch08-v3-fullpath@blackdark.local", "tier": "pro"}
...
async def run_full_path_performance() -> dict[str, Any]:
    results = [await _benchmark_full_path(cid) for cid in BATCH08_IDS]
    ...
    return {
        "artifact": "BATCH08_FULL_PATH_PERFORMANCE",
...
def run_full_path_entitlement_pytest(baseline_head: str) -> dict[str, Any]:
    proc = subprocess.run(
        [sys.executable, "-m", "pytest", "tests/test_batch08_full_path_entitlement.py", "-q", "--tb=short"],
        cwd=ROOT,
        capture_output=True,
        text=True,
    )
    ...
    return {
        "artifact": "BATCH08_FULL_PATH_ENTITLEMENT",
        "generated_at": datetime.now(UTC).isoformat(),
        "git_commit": baseline_head,
        "test_module": "tests/test_batch08_full_path_entitlement.py",
...
def build_event_loop_forensic(baseline_head: str) -> dict[str, Any]:
    ...
    return {
        "artifact": "BATCH08_EVENT_LOOP_FORENSIC",
        "generated_at": datetime.now(UTC).isoformat(),
        "git_commit": baseline_head,
...
def build_ssot_reconciliation(baseline_head: str) -> dict[str, Any]:
    ...
    batch08_in_inv = [int(k) for k in inv.get("per_id", {}) if 301 <= int(k) <= 350]
    inv_statuses = Counter(inv.get("per_id", {}).get(str(i), {}).get("status") for i in BATCH08_IDS)

    return {
        "artifact": "BATCH08_SSOT_RECONCILIATION",
        "generated_at": datetime.now(UTC).isoformat(),
        "git_commit": baseline_head,
        "trackers": {
            "PROGRESS_826_CANONICAL": {
                ...
                "batch08_note": (
                    "Batch08 351-400 are PASS_ENGINEERING / LOCAL_COMPLETE — not PRODUCTION-ALIGNED. "
                    "Numerator unchanged until G6/Railway. Inventory lists batch08 IDs as PENDING/DEFERRED."
                ),
            },
            "CAPABILITIES_826_INVENTORY": {
                ...
                "batch08_ids_present": len(batch08_in_inv),
                "batch08_status_snapshot": dict(inv_statuses),
            },
            "batch08_independent": {
                "exists": False,
                "active_canonical_role": "NOT_PRESENT",
                "replacement_ssot": "docs/BATCH08_FINAL_LOCAL_FREEZE.json + docs/BATCH08_V3_STATE_CLASSIFICATION.json",
                "note": "No batch08_independent counter required — v3 freeze is batch08 SSOT",
            },
        },
        "stale_active_ssot": [],
...
def fetch_sonar_gate_evidence(head: str, branch: str = "cursor/batch08-351-400-ed16") -> dict[str, Any]:
    proc = subprocess.run(
        [
            "gh",
            "run",
