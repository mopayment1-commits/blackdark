#!/usr/bin/env python3
"""Batch09 v3 reconciliation — state vocabulary, duplicate coverage, hero matrix, SSOT, forensics."""
...
BATCH09_IDS = recon.BATCH09_IDS
EXPECTED_COUNT = recon.EXPECTED_COUNT
PRIOR_IDS = list(range(1, 401))
EXPECTED_CROSS_BATCH_PAIRS = len(BATCH09_IDS) * len(PRIOR_IDS)
...
def build_v3_state_classification(
    ...
    for cid in BATCH09_IDS:
        ...
        if cid in recon.REUSED_LINK_CATALOG:
            ...
            if mod.endswith("defi_yield_intelligence_layer") or mod.endswith("quicktake_feed"):
        ...
        rows.append(
            {
                ...
                "binding": f"{mod}.{fn}",
                "evidence": f"docs/RETROSPECTIVE_DEEP_AUDIT_BATCH_05_401_500.json#capability_id={cid}",
            }
        )

    counts = Counter(r["canonical_state"] for r in rows)
    return {
        "artifact": "BATCH09_V3_STATE_CLASSIFICATION",
...
def _pair_decision(
    ...
    if batch09_id in recon.REUSED_LINK_CATALOG and prior_id == recon.REUSED_LINK_CATALOG[batch09_id]["canonical_capability_id"]:
        return {
...
def build_cross_batch_exhaustive_coverage(
                ...
                material_overlaps.append(
                    {
                        ...
                        "batch09_binding": f"{b_bind[0]}.{b_bind[1]}",
    ...
    return {
        "scope": "Exhaustive machine coverage — Batch09 401-450 vs prior 1-350",
        "method": (
            "Deterministic pair evaluator over all 50×300 candidate pairs. "
            "No indexing filter may omit pairs — each (batch09_id, prior_id) receives a decision. "
        ...
        "candidate_filter_false_negative_risk": (
            ...
            "Material duplicate detection uses binding equality, canonical reuse (#437→#288), and normalized name overlap."
        ),
        "overlap_dimensions_reviewed": [
            ...
            "route/API overlap (unique pdf_capability_registry bindings per batch09 ID)",
            "data/provider overlap (defi_yield_intelligence_layer shared core — SHARED_MODULE_DISTINCT_FN)",
            "model overlap (no shared AI model binding collisions detected)",
            "entitlement overlap (cap646 gate — per-ID routing)",
            "Hero contribution overlap (#437 facade CONTEXT only; no FEED double-count)",
        ],
...
def _hero_cell(cid: int, hero: dict[str, str], catalog_name: str, track: str) -> dict[str, Any]:
    ...
    if cid == 437 and hid == "H2":
        return {
            "relationship": "CONTEXT",
            "reason": "Facade delegates to canonical #288 mindshare correlation — contextual reuse, not additive Hero FEED",
            "evidence_location": "docs/BATCH09_DUPLICATE_CANONICAL_ANALYSIS.json#437_to_288",
        }
    if cid == 441 and hid == "H1":
        return {
            "relationship": "CONTEXT",
            "reason": "Strategy vetting facade delegates to canonical #155 stat-arb — opportunity framing context only",
            "evidence_location": "docs/BATCH09_SIX_HEROES_BINDING.json#capability_id=441",
        }
    if cid in (413, 416, 435, 436) and hid == "H1":
        return {
            "relationship": "CONTEXT",
            "reason": "DeFi opportunity screener/risk surfaces — opportunity framing context, not direct FEED",
            "evidence_location": f"docs/BATCH09_HERO_MATRIX_401_450.json#capability_id={cid}#{hid}",
        }
    if "yield" in name_l or "defi" in name_l:
        if hid == "H4":
            return {
                "relationship": "CONTEXT",
                "reason": "Yield/portfolio intelligence informs Portfolio AI context — not direct FEED",
                "evidence_location": f"docs/BATCH09_HERO_MATRIX_401_450.json#capability_id={cid}#{hid}",
            }
    return {
        "relationship": "NOT_APPLICABLE",
        "reason": "DeFi/yield intelligence surface — quiet engine; no direct Six Hero FEED per HEROES_STRATEGY_BINDING",
        "evidence_location": f"docs/BATCH09_HERO_MATRIX_401_450.json#capability_id={cid}#{hid}",
    }
...
def build_hero_matrix_50x6(
        ...
        for hero in CANONICAL_HEROES:
            ...
            if cell["relationship"] == "FEED":
                cell["feed_proof"] = {
                    ...
                    "double_count_review": "no duplicate Hero score — CONTEXT/NA default for batch09 defi/yield",
                }
            cells.append(cell)
    ...
    if any(c["capability_id"] == 437 and c["relationship"] == "FEED" for c in cells):
        duplicate_contribs.append("437_feed")
    if any(c["capability_id"] == 441 and c["relationship"] == "FEED" for c in cells):
        duplicate_contribs.append("441_feed")
    ...
    return {
        "artifact": "BATCH09_HERO_MATRIX_401_450",
        "generated_at": datetime.now(UTC).isoformat(),
        "git_commit": baseline_head,
        ...
        "summary": {
            ...
            "437_no_double_count_with_288": "437" not in duplicate_contribs,
            "441_no_hero_double_count": "441" not in duplicate_contribs,
        },
        "cells": cells,
    }
...
async def _benchmark_full_path(cid: int) -> dict[str, Any]:
    ...
    user = {"id": 1, "email": "batch09-v3-fullpath@blackdark.local", "tier": "pro"}
...
async def run_full_path_performance() -> dict[str, Any]:
    results = [await _benchmark_full_path(cid) for cid in BATCH09_IDS]
    ...
    class_summary = {
        cls: {
            "count": len(rows),
            "p95_ms_max": round(max(r["p95_ms"] for r in rows if "p95_ms" in r), 2)
            if any("p95_ms" in r for r in rows)
            else None,
            "p99_ms_max": round(max(r["p99_ms"] for r in rows if "p99_ms" in r), 2)
            if any("p99_ms" in r for r in rows)
            else None,
        }
        for cls, rows in by_class.items()
    }
    return {
        "artifact": "BATCH09_FULL_PATH_PERFORMANCE",
...
def run_full_path_entitlement_pytest(baseline_head: str) -> dict[str, Any]:
    proc = subprocess.run(
        [sys.executable, "-m", "pytest", "tests/test_batch09_full_path_entitlement.py", "-q", "--tb=short"],
        cwd=ROOT,
        capture_output=True,
        text=True,
    )
    ...
    return {
        "artifact": "BATCH09_FULL_PATH_ENTITLEMENT",
        "generated_at": datetime.now(UTC).isoformat(),
        "git_commit": baseline_head,
        "test_module": "tests/test_batch09_full_path_entitlement.py",
...
def build_event_loop_forensic(baseline_head: str) -> dict[str, Any]:
    suites = [
        ...
        ("batch09_hero_capabilities", "tests/test_hero_batch_09_capabilities.py", "Unclosed client session"),
    ]
    ...
    return {
        "artifact": "BATCH09_EVENT_LOOP_FORENSIC",
        "generated_at": datetime.now(UTC).isoformat(),
        "git_commit": baseline_head,
...
def build_ssot_reconciliation(baseline_head: str) -> dict[str, Any]:
    ...
    batch09_in_inv = [int(k) for k in inv.get("per_id", {}) if 401 <= int(k) <= 450]
    inv_statuses = Counter(inv.get("per_id", {}).get(str(i), {}).get("status") for i in BATCH09_IDS)
    ...
    return {
        ...
        "trackers": {
            "PROGRESS_826_CANONICAL": {
                ...
                "batch09_note": (
                    "Batch09 401-450 are PASS_ENGINEERING / LOCAL_COMPLETE — not PRODUCTION-ALIGNED. "
                    "Numerator unchanged until G6/Railway. Inventory lists batch09 IDs as PENDING/DEFERRED."
                ),
            },
            "CAPABILITIES_826_INVENTORY": {
                ...
                "batch09_ids_present": len(batch09_in_inv),
                "batch09_status_snapshot": dict(inv_statuses),
            },
            "batch09_independent": {
                "exists": False,
                "active_canonical_role": "NOT_PRESENT",
                "replacement_ssot": "docs/BATCH09_FINAL_LOCAL_FREEZE.json + docs/BATCH09_V3_STATE_CLASSIFICATION.json",
                "note": "No batch09_independent counter required — v3 freeze is batch09 SSOT",
            },
        },
        "stale_active_ssot": [],
...
def fetch_sonar_gate_evidence(head: str, branch: str = "cursor/batch09-401-450-ed16") -> dict[str, Any]:
    proc = subprocess.run(
        [
            "gh",
            "run",
...
PROVEN_NON_ACTIONABLE_SUITE_RULES: list[dict[str, Any]] = [
    ...
    {
        "suite_label": "batch09_hero_capabilities",
        "match_output": ("Unclosed client session", "aiohttp"),
        "classification": "TEST_HARNESS_ARTIFACT",
        "root_cause": (
            "aiohttp ClientSession GC teardown during exchange_currency_status CCXT probes in hero batch09; "
