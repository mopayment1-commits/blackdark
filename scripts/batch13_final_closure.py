#!/usr/bin/env python3
"""Batch13 institutional closure — capabilities 601-650 (operational market intelligence)."""
...
from bd_platform.batch13_membership import (  # noqa: E402
    BATCH13_IDS,
...
DOMAIN_SPEC_CAPABILITY_TYPE = "operational_market_intelligence"
DOMAIN_SPEC_APPLICABILITY_DELTA: dict[str, dict[str, str]] = {
    ...
    "signal_registry": {
        ...
        "rationale": "Batch13 institutional delivery analytics are read-only intelligence — no signal emission registry per ID",
    },
    "prediction_ledger": {
        "applicability": "NOT_APPLICABLE",
    ...
    "algorithm_model_versioning": {
        "applicability": "PARTIALLY_APPLICABLE",
        "rationale": "Scoring/heuristics versioned at batch13_operational_intelligence_layer; not per-capability model registry",
    },
...
TEMPORAL_CONTROLS_DELTA: dict[str, dict[str, str]] = {
    ...
    "deterministic_mass_replay_engine": {
        "applicability": "NOT_APPLICABLE",
        "rationale": "Batch13 surfaces are analytics payloads — no mass replay engine obligation per ID",
    },
...
SECURITY_CHECKS = [
    ...
    ("authorization", "PROVEN_LOCAL", "tests/test_batch13_consumer_paths.py execute probe"),
    ...
    ("malformed_input", "PROVEN_LOCAL", "symbol/seed normalization in batch13_operational_intelligence_layer builders"),
...
def build_layer_a_internal_pairwise(
        ...
        per_id_rows.append(
            {
                ...
                "note": "Layer A scope is intra-Batch13 only; #601 canonical duplicate is outside shared core",
            }
        )

    shared_mod = "bd_platform.batch13_operational_intelligence_layer"
    ...
    internal_unresolved = sum(
        ...
        if not all(p in CANONICAL_DECISIONS or p in external_dependency_ids() for p in alias["capability_ids"])
    )

    return {
        "scope": "Layer A — Batch13 IDs 601-650 vs 601-650 ONLY",
                ...
                material_overlaps.append(
                    {
                        ...
                        "batch13_binding": f"{b_bind[0]}.{b_bind[1]}",
        ...
        rows.append(
            {
                ...
                "evidence": f"pdf_capability_registry binding unique for batch13 #{cid}",
                "decision": decision,
...
def neighbor_distinction(cap_id: int) -> str:
    ...
    if idx <= 0:
        return "First shared-core parameterized ID in batch13 institutional delivery layer"
    prev_id = peers[idx - 1]
        ...
        rows.append(
            {
                ...
                "independent_oracle_invariant": (
                    ...
                    "tests/test_batch13_independent_oracle_semantics.py"
                ),
                ...
                "shared_core_component": "bd_platform.batch13_operational_intelligence_layer + batch13_semantic_engine",
                "exact_semantic_distinction_vs_neighbors": neighbor_distinction(cap_id),
                "classification": "KEEP_DISTINCT_BUT_REUSE_SHARED_CORE",
                "test_evidence_reference": "tests/test_batch13_shared_core_semantics.py",
                "tested_sha": head,
            }
        )
    return rows
        ...
        rows.append(
            {
                ...
                "oracle_source": "tests/batch13_independent_semantic_oracles.py",
                "production_source": "bd_platform.batch13_semantic_engine",
        ...
        rows.append(
            {
                ...
                "test_evidence": [
                    "tests/test_batch13_consumer_paths.py",
                    "tests/test_institutional_delivery_intelligence_batch601_650.py",
                ],
...
def build_capability_inventory(
    ...
    return {
        "artifact": "BATCH13_CAPABILITY_INVENTORY_601_650",
...
def build_rtm(
        ...
        rows.append(
            {
                ...
                "test": "tests/test_batch13_consumer_paths.py",
    ...
    return {
        "artifact": "BATCH13_RTM_601_650",
        "generated_at_utc": datetime.now(UTC).isoformat(),
        "git_commit": head,
        "scope": "Requirements traceability matrix — Batch13 601-650",
        "rows": rows,
        ...
        per_id.append(
            {
                ...
                "partial_reuse_rationale": (
                    "Shared batch13_operational_intelligence_layer infrastructure with distinct parameterized semantics"
                    if cid in shared_core_ids_list()
                    else "Hero facade canonical reuse to #74 — outside shared core"
                ),
            }
        )
    return {
        "artifact": "BATCH13_EXISTING_PARTIAL_REUSE_ANALYSIS",
        "generated_at_utc": datetime.now(UTC).isoformat(),
        "git_commit": head,
        "scope": "v4_v2 domain spec + Temporal Intelligence controls — Batch13 601-650",
...
def build_domain_spec_applicability(head: str) -> dict[str, Any]:
    ...
    return {
        "artifact": "BATCH13_DOMAIN_SPEC_APPLICABILITY",
        "generated_at_utc": datetime.now(UTC).isoformat(),
        "git_commit": head,
        "domain_spec_authority": str(DOMAIN_SPEC_PATH.relative_to(ROOT)),
        "scope": "Delta-only NOT_APPLICABLE / PARTIALLY_APPLICABLE mapping — operational_market_intelligence 601-650",
        "capability_type": DOMAIN_SPEC_CAPABILITY_TYPE,
...
def build_temporal_applicability(head: str) -> dict[str, Any]:
    ...
    return {
        "artifact": "BATCH13_TEMPORAL_INTELLIGENCE_APPLICABILITY",
        "generated_at_utc": datetime.now(UTC).isoformat(),
        "git_commit": head,
        "temporal_spec_authority": str(TEMPORAL_SPEC_PATH.relative_to(ROOT)),
        "scope": "Temporal Intelligence control delta mapping — Batch13 601-650",
...
def build_security_audit(head: str) -> dict[str, Any]:
    return {
        "artifact": "BATCH13_SECURITY_MATERIAL_PATH_AUDIT",
        "generated_at_utc": datetime.now(UTC).isoformat(),
        "git_commit": head,
        ...
        rows.append(
            {
                ...
                "status": "PROVEN_LOCAL",
                "evidence": "tests/test_batch13_consumer_paths.py",
            }
        )
    return {
        "artifact": "BATCH13_ENTITLEMENT_AUDIT",
        "generated_at_utc": datetime.now(UTC).isoformat(),
        "git_commit": head,
        "scope": "Entitlement and object-level authorization — Batch13 601-650",
...
def build_ssot_reconciliation(head: str) -> dict[str, Any]:
    return {
        "artifact": "BATCH13_SSOT_RECONCILIATION",
        "generated_at_utc": datetime.now(UTC).isoformat(),
        "git_commit": head,
...
async def main() -> int:
    ...
    created.append(
        write_json(
            "BATCH13_CAPABILITY_INVENTORY_601_650.json",
    ...
    created.append(
        write_json(
            "BATCH13_INTERNAL_DUPLICATE_ANALYSIS.json",
            {
                "artifact": "BATCH13_INTERNAL_DUPLICATE_ANALYSIS",
    ...
    created.append(
        write_json(
            "BATCH13_CROSS_BATCH_DUPLICATE_ANALYSIS.json",
            {
                "artifact": "BATCH13_CROSS_BATCH_DUPLICATE_ANALYSIS",
                "generated_at_utc": now,
                "git_commit": head,
    ...
    created.append(
        write_json(
            "BATCH13_EXISTING_PARTIAL_REUSE_ANALYSIS.json",
    ...
    created.append(
        write_json(
            "BATCH13_CANONICAL_DECISIONS.json",
            {
                "artifact": "BATCH13_CANONICAL_DECISIONS",
                "generated_at_utc": now,
                "git_commit": head,
    ...
    created.append(write_json("BATCH13_RTM_601_650.json", build_rtm(bindings, catalog, head)).name)

    created.append(
        write_json(
            "BATCH13_SEMANTIC_EVIDENCE.json",
            {
                "artifact": "BATCH13_SEMANTIC_EVIDENCE",
                "generated_at_utc": now,
                "git_commit": head,
    ...
    created.append(
        write_json(
            "BATCH13_INDEPENDENT_ORACLE_EVIDENCE.json",
            {
                "artifact": "BATCH13_INDEPENDENT_ORACLE_EVIDENCE",
                "generated_at_utc": now,
                "git_commit": head,
    ...
    created.append(
        write_json(
            "BATCH13_CONSUMER_PATH_EVIDENCE.json",
            {
                "artifact": "BATCH13_CONSUMER_PATH_EVIDENCE",
                "generated_at_utc": now,
                "git_commit": head,
    ...
    created.append(
        write_json("BATCH13_DOMAIN_SPEC_APPLICABILITY.json", build_domain_spec_applicability(head)).name
    )
    created.append(
        write_json("BATCH13_TEMPORAL_INTELLIGENCE_APPLICABILITY.json", build_temporal_applicability(head)).name
    )
    created.append(write_json("BATCH13_SECURITY_MATERIAL_PATH_AUDIT.json", build_security_audit(head)).name)
    created.append(write_json("BATCH13_ENTITLEMENT_AUDIT.json", build_entitlement_audit(head)).name)

    created.append(
        write_json(
            "BATCH13_CROSS_BATCH_REGRESSION.json",
            {
                "artifact": "BATCH13_CROSS_BATCH_REGRESSION",
                "generated_at_utc": now,
                "git_commit": head,
                "suites": regression,
    ...
    created.append(
        write_json(
            "BATCH13_FORMAL_GATE_PROVENANCE.json",
            {
                "artifact": "BATCH13_FORMAL_GATE_PROVENANCE",
                "generated_at_utc": now,
                "final_code_sha": head,
    ...
    created.append(write_json("BATCH13_SSOT_RECONCILIATION.json", build_ssot_reconciliation(head)).name)

    shared_count = len(shared_core_ids())
    freeze_ok = (
        ...
        and len(semantic_rows) == shared_count
        and len(oracle_rows) == shared_count
    ...
    flags = {
        ...
        "CROSS_BATCH_PAIRS_REVIEWED_30000": exhaustive["evaluated_cross_batch_pairs"] >= EXPECTED_CROSS_BATCH_PAIRS,
        ...
        "CANONICAL_DECISION_613_TO_88": True,
    ...
    created.append(
        write_json(
            "BATCH13_FINAL_LOCAL_FREEZE.json",
            {
                "artifact": "BATCH13_FINAL_LOCAL_FREEZE",
                "generated_at_utc": now,
                "final_head": head,
                "branch": BRANCH,
                "capability_range": "601-650",
                "formal_gate_provenance": "docs/BATCH13_FORMAL_GATE_PROVENANCE.json",
                "semantic_evidence": "docs/BATCH13_SEMANTIC_EVIDENCE.json",
                "independent_oracle_evidence": "docs/BATCH13_INDEPENDENT_ORACLE_EVIDENCE.json",
                "consumer_paths": "docs/BATCH13_CONSUMER_PATH_EVIDENCE.json",
                "regression": "docs/BATCH13_CROSS_BATCH_REGRESSION.json",
                "ssot_reconciliation": "docs/BATCH13_SSOT_RECONCILIATION.json",
                "flags": flags,
                "preserved_statuses": {
