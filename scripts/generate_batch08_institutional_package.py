#!/usr/bin/env python3
"""Generate Batch08 institutional JSON artifacts for capabilities 351-400.
...
from scripts import batch08_v3_reconciliation as v3  # noqa: E402

BATCH08_IDS = list(range(351, 401))
EXPECTED_COUNT = 50
CATALOG_PATH = ROOT / "docs/cap646/CAP646_CATALOG.json"
AUDIT_PATH = ROOT / "docs/docs/RETROSPECTIVE_DEEP_AUDIT_BATCH_04_301_400.json"
MANIFEST_PATH = ROOT / "scripts/partial_batches/batch_08_351_400.json"
DOCS = ROOT / "docs"

REUSED_LINK_CATALOG: dict[int, dict[str, Any]] = {
    379: {
        "decision": "REUSED-LINK",
        "canonical_capability_id": 189,
        "canonical_spine": "batch04",
        "underlying_module": "bd_platform.arbitrage_portfolio_ux_layer",
        "underlying_function": "analyze_liquidity_capacity_189",
        "binding": "bd_platform.arbitrage_portfolio_ux_layer.analyze_liquidity_capacity_189",
        "hero_context": "Liquidity capacity reuse via canonical #189",
    },
}
...
SECURITY_CHECKS = [
    ...
    ("authorization", "PROVEN_LOCAL", "tests/test_hero_batch_08_capabilities.py execute probe"),
...
RAILWAY_QUEUE = [
    ...
    {
        "id": "RL2",
        "item": "Gate Zero live health + cap646 probes (351-400)",
    ...
    {
        "id": "RL5",
        "item": "Per-ID PASS_LIVE elevation (351-400)",
...
LOCAL_COMPLETE_QUEUE = [
    {"category": "G0-G5", "status": "50/50 PASS_ENGINEERING", "evidence": "docs/BATCH08_PER_ID_FINAL_MATRIX_351_400.json"},
    {"category": "Duplicate/canonical", "status": "1 REUSED-LINK / 49 KEEP_DISTINCT / 0 conflicts"},
    {"category": "Security material paths", "status": "PROVEN_LOCAL", "evidence": "docs/BATCH08_SECURITY_MATERIAL_PATH_AUDIT.json"},
    {"category": "Data integrity", "status": "50/50 PROVEN_LOCAL", "evidence": "docs/BATCH08_DATA_QUALITY_INTEGRITY.json"},
    {"category": "Reliability", "status": "PROVEN_LOCAL", "evidence": "docs/BATCH08_RELIABILITY_FAILURE_MODES.json"},
    {"category": "Observability local", "status": "COMPLETE_LOCAL", "evidence": "docs/BATCH08_OBSERVABILITY_READINESS.json"},
    {"category": "Performance prep", "status": "LOCAL_PREPARED", "evidence": "docs/BATCH08_PERFORMANCE_CAPACITY_PREP.json"},
    {"category": "Six Heroes", "status": "379 HERO_CONTEXT_ONLY; 49 NOT_HERO_RELEVANT"},
    {"category": "12207 Validation prep", "status": "LOCAL_COMPLETE", "evidence": "docs/BATCH08_12207_VALIDATION_PACKAGE.json"},
    ...
    {"category": "Cross-batch regression", "status": "FULL_PASS", "evidence": "docs/BATCH08_CROSS_BATCH_REGRESSION.json"},
]
...
OUTPUT_FILES = [
    "BATCH08_BASELINE.json",
    "BATCH08_DUPLICATE_CANONICAL_ANALYSIS.json",
    "BATCH08_RTM_351_400.json",
    "BATCH08_PER_ID_FINAL_MATRIX_351_400.json",
    "BATCH08_PENTAGONAL_TEMPLATE_351_400.json",
    "BATCH08_PENTAGONAL_COL10_PREPARATION.json",
    "BATCH08_DATA_QUALITY_INTEGRITY.json",
    "BATCH08_SECURITY_MATERIAL_PATH_AUDIT.json",
    "BATCH08_RELIABILITY_FAILURE_MODES.json",
    "BATCH08_OBSERVABILITY_READINESS.json",
    "BATCH08_PERFORMANCE_CAPACITY_PREP.json",
    "BATCH08_SIX_HEROES_BINDING.json",
    "BATCH08_12207_VALIDATION_PACKAGE.json",
    "BATCH08_12207_TRANSITION_PACKAGE.json",
    "BATCH08_12207_OPERATION_READINESS_PACKAGE.json",
    "BATCH08_SRE_PRR_PACKAGE.json",
    "BATCH08_G7_PRE_ASSURANCE_PACKAGE.json",
    "BATCH08_CROSS_BATCH_REGRESSION.json",
    "BATCH08_STATUS_QUEUES.json",
    "BATCH08_EXISTING_VERIFIED_EVIDENCE.json",
    "BATCH08_COLLECTIVE_REVIEW_LOCAL.json",
    "BATCH08_V3_STATE_CLASSIFICATION.json",
    "BATCH08_FULL_PATH_ENTITLEMENT.json",
    "BATCH08_FULL_PATH_PERFORMANCE.json",
    "BATCH08_HERO_MATRIX_351_400.json",
    "BATCH08_EVENT_LOOP_FORENSIC.json",
    "BATCH08_SSOT_RECONCILIATION.json",
    "BATCH08_FINAL_LOCAL_FREEZE.json",
]
...
def tested_source_head() -> str:
    """Return last commit that touched Batch08 implementation (not docs-only freeze)."""
    impl_paths = [
        "tests/test_hero_batch_08_capabilities.py",
        "scripts/run_batch08_deep_closure.py",
        "scripts/partial_batches/batch_08_351_400.json",
    ]
...
def load_catalog() -> dict[int, dict[str, Any]]:
    ...
    catalog = {int(r["id"]): r for r in rows if int(r["id"]) in BATCH08_IDS}
    if len(catalog) != EXPECTED_COUNT:
        raise SystemExit(f"catalog: expected {EXPECTED_COUNT} rows for 351-400, got {len(catalog)}")
    return catalog
...
def load_audit(bindings: dict[int, tuple[str, str]], catalog: dict[int, dict[str, Any]]) -> dict[str, Any]:
        ...
        rows.append(
            {
                ...
                "independent_test_file": "tests/test_hero_batch_08_capabilities.py",
    ...
    return {
        ...
        "placeholder": True,
        "note": "Minimal placeholder — run scripts/run_batch08_deep_closure.py for full quad audit",
...
def audit_row_by_id(audit: dict[str, Any]) -> dict[int, dict[str, Any]]:
    return {
        ...
        if BATCH08_IDS[0] <= int(r["capability_id"]) <= BATCH08_IDS[-1]
    }
...
def classify_internal_decision(cid: int, bindings: dict[int, tuple[str, str]]) -> str:
    """Layer A only — intra-Batch08 binding uniqueness."""
    mod, fn = bindings[cid]
    peers = [other for other in BATCH08_IDS if other != cid and bindings[other] == (mod, fn)]
    if peers:
...
def build_duplicate_analysis(
    ...
    return {
        ...
        "artifact": "BATCH08_DUPLICATE_CANONICAL_ANALYSIS",
        "generated_at": datetime.now(UTC).isoformat(),
        "git_commit": baseline_head,
        "scope": "Two-layer duplicate/canonical analysis — Layer A intra-Batch08; Layer B vs 1-350",
        "method": (
            "Layer A: pairwise 351-400 binding/semantic review (no cross-batch IDs). "
        ...
        "decisions": {
            "379_to_189": recon.REUSED_LINK_CATALOG[379],
            "382_hero_facade": recon.HERO_FACADE_DELEGATIONS[382],
        },
    }
...
def build_baseline(
    ...
    missing = [cid for cid in BATCH08_IDS if cid not in bindings]
    duplicate_ids: list[int] = []
    pair_counts = Counter(bindings[cid] for cid in BATCH08_IDS)
    for cid in BATCH08_IDS:
    ...
    return {
        "machine_assertions": build_machine_assertions(
            ...
            quad_pass_count=sum(1 for cid in BATCH08_IDS if quad_pass(audit_by[cid].get("classification", ""))),
        ),
        "artifact": "BATCH08_BASELINE",
        "phase": 0,
        ...
        "provenance": {
            ...
            "description": (
                ...
                "(scripts/run_batch08_deep_closure.py ancestry). "
        ...
        "inventory": {
            ...
            "binding_coverage": len(BATCH08_IDS) - len(missing),
        },
...
def build_rtm(
        ...
        rows.append(
            {
                ...
                "test": "tests/test_hero_batch_08_capabilities.py",
    ...
    return {
        ...
        "artifact": "BATCH08_RTM_351_400",
        "generated_at": datetime.now(UTC).isoformat(),
        "git_commit": baseline_head,
        "scope": "Requirements traceability matrix — Batch08 351-400",
        "rows": rows,
...
def build_per_id_matrix(
    ...
    return {
        ...
        "artifact": "BATCH08_PER_ID_FINAL_MATRIX_351_400",
        "generated_at": datetime.now(UTC).isoformat(),
        "git_commit": baseline_head,
...
def build_pentagonal_template(
        ...
        rows.append(
            {
                ...
                "pentagonal": {
                    ...
                    "col2_external_result_iso29148": {
                        ...
                        "verification": "tests/test_hero_batch_08_capabilities.py",
                        "match": quad_pass(audit_by[cid].get("classification", "")),
                    },
                    "col3_interface_iso29119": {
                        "api_path": f"/api/cap646/{cid}",
                        "local_tests": ["tests/test_hero_batch_08_capabilities.py"],
                    ...
                    "col5_readiness_evidence": {
                        ...
                        "evidence": "docs/BATCH08_EXISTING_VERIFIED_EVIDENCE.json",
                        "evidence_location": f"docs/BATCH08_PENTAGONAL_TEMPLATE_351_400.json#capability_id={cid}",
    ...
    return {
        ...
        "artifact": "BATCH08_PENTAGONAL_TEMPLATE_351_400",
        "generated_at": datetime.now(UTC).isoformat(),
        "git_commit": baseline_head,
        "scope": "Pentagonal template cols 1-5 per ID — Batch08 351-400",
        "rows": rows,
    }
    ...
    rows = [
        {
            ...
            "checklist_ref": "docs/BATCH08_PENTAGONAL_TEMPLATE_351_400.json",
        }
        for cid in BATCH08_IDS
    ]
    return {
        "machine_assertions": build_machine_assertions(col10_prepared=len(rows), col10_complete=len(rows)),
        "artifact": "BATCH08_PENTAGONAL_COL10_PREPARATION",
        "generated_at": datetime.now(UTC).isoformat(),
        "git_commit": baseline_head,
...
def build_data_quality(audit: dict[str, Any], baseline_head: str) -> dict[str, Any]:
    return {
        ...
        "artifact": "BATCH08_DATA_QUALITY_INTEGRITY",
        "generated_at": datetime.now(UTC).isoformat(),
        "git_commit": baseline_head,
        "status": "PROVEN_LOCAL",
...
def build_security_audit(baseline_head: str) -> dict[str, Any]:
    return {
        ...
        "artifact": "BATCH08_SECURITY_MATERIAL_PATH_AUDIT",
        "generated_at": datetime.now(UTC).isoformat(),
        "git_commit": baseline_head,
        "status": "COMPLETE_LOCAL",
...
def build_reliability(baseline_head: str) -> dict[str, Any]:
    return {
        ...
        "artifact": "BATCH08_RELIABILITY_FAILURE_MODES",
        "generated_at": datetime.now(UTC).isoformat(),
        "git_commit": baseline_head,
        "status": "PROVEN_LOCAL",
        "failure_modes": [
...
def build_observability(baseline_head: str) -> dict[str, Any]:
    return {
        ...
        "artifact": "BATCH08_OBSERVABILITY_READINESS",
        "generated_at": datetime.now(UTC).isoformat(),
        "git_commit": baseline_head,
...
def build_performance_prep(
    ...
    doc = {
        ...
        "artifact": "BATCH08_PERFORMANCE_CAPACITY_PREP",
        "generated_at": datetime.now(UTC).isoformat(),
        "git_commit": baseline_head,
        "performance_tiers": {
...
def build_six_heroes(
    ...
    for cid in BATCH08_IDS:
        ...
        if cid == 382:
            entry["hero_facade"] = {
                ...
                "duplicate_hero_contribution": False,
                "note": recon.HERO_FACADE_DELEGATIONS[382]["hero_impact"],
            }
        per_id.append(entry)
    ...
    return {
        ...
        "artifact": "BATCH08_SIX_HEROES_BINDING",
        "generated_at": datetime.now(UTC).isoformat(),
        "git_commit": baseline_head,
        "scope": "Six Heroes binding classification — Batch08 351-400 (post duplicate reconciliation)",
        "summary": {
            "NOT_HERO_RELEVANT": EXPECTED_COUNT - 1,
            "HERO_CONTEXT_ONLY": 1,
            "HERO_DIRECT_FEED": 0,
        ...
        "reconfirmation": {
            "379_no_double_count_with_189": True,
            "382_facade_distinct_no_double_count": True,
...
def build_validation_package(
    ...
    return {
        ...
        "artifact": "BATCH08_12207_VALIDATION_PACKAGE",
        "generated_at": datetime.now(UTC).isoformat(),
        "git_commit": baseline_head,
        "scope": "Batch08 12207 Validation Package IDs 351-400",
...
def build_transition_package(baseline_head: str) -> dict[str, Any]:
    return {
        ...
        "artifact": "BATCH08_12207_TRANSITION_PACKAGE",
        "generated_at": datetime.now(UTC).isoformat(),
        "git_commit": baseline_head,
        "scope": "Batch08 12207 Transition Package",
        "status": "TRANSITION_PREPARED_LOCAL",
        "not_claimed": ["TRANSITION_EXECUTED_LIVE"],
        "deployment_prerequisites": [
            "Railway app with BLACKDARK production env",
            "pdf_capability_registry bindings 351-400 wired",
            "Entitlement provider configured",
        ],
        "smoke_test_spec": "GET /api/cap646/{351..400} sample + health endpoints",
        ...
        "ownership": "batch08-institutional-owner",
    }
...
def build_operation_package(baseline_head: str) -> dict[str, Any]:
    return {
        ...
        "artifact": "BATCH08_12207_OPERATION_READINESS_PACKAGE",
        "generated_at": datetime.now(UTC).isoformat(),
        "git_commit": baseline_head,
        "scope": "Batch08 12207 Operation Readiness",
        "status": "OPERATION_PREPARED_LOCAL",
        "not_claimed": ["OPERATION_PROVEN_LIVE"],
        "operational_ownership": "batch08-institutional-owner",
...
def build_sre_prr(baseline_head: str) -> dict[str, Any]:
    return {
        ...
        "artifact": "BATCH08_SRE_PRR_PACKAGE",
        "generated_at": datetime.now(UTC).isoformat(),
        "git_commit": baseline_head,
        "scope": "Batch08 SRE Production Readiness Review",
...
def build_g7_package(baseline_head: str) -> dict[str, Any]:
    return {
        ...
        "artifact": "BATCH08_G7_PRE_ASSURANCE_PACKAGE",
        "generated_at": datetime.now(UTC).isoformat(),
        "git_commit": baseline_head,
        "scope": "Batch08 G7 Pre-Assurance Package",
        "status": "G7_LOCAL_PREPARATION_COMPLETE",
        "remaining": "G7_INDEPENDENT_SIGNOFF_PENDING",
        "not_claimed": ["G7 PASS", "ASSURANCE_READY"],
        "traceability": "RTM 351-400 → per-ID matrix → duplicate analysis",
        "reviewer_checklist": [
...
def build_cross_batch_regression(baseline_head: str) -> dict[str, Any]:
    ...
    suites.append(
        {
            "label": "batch08_binding_coverage",
            "passed": all(
                s.get("passed") for s in suites if s["label"] == "batch08_hero_capabilities"
            ),
            "note": "50/50 discover_bindings() present for 351-400",
        }
    )
    ...
    return {
        ...
        "artifact": "BATCH08_CROSS_BATCH_REGRESSION",
        "generated_at": datetime.now(UTC).isoformat(),
        "git_commit": baseline_head,
...
def build_final_freeze(
    ...
    *,
    ...
    if exhaustive.get("evaluated_cross_batch_pairs") != 17500:
        deficiencies.append("cross_batch_pairs_not_17500")
    ...
    return {
        "artifact": "BATCH08_FINAL_LOCAL_FREEZE",
        ...
        "provenance": {
            ...
            "description": (
                ...
                "Derive via: git log -1 --format=%H -- docs/BATCH08_FINAL_LOCAL_FREEZE.json. "
                "The artifact does NOT embed its own commit SHA as an institutional correctness gate."
            ),
            "self_referential_head_embedding": "prohibited",
        },
        ...
        "preserved": {
            ...
            "batch08_capability_range": "351-400",
            "railway_execution": "deferred",
            "node24_actions": True,
        },
        "batch08_classification": {
        ...
        "canonical_definition_source": {
            ...
            "manifest": "scripts/partial_batches/batch_08_351_400.json",
...
def build_status_queues(baseline_head: str) -> dict[str, Any]:
    return {
        ...
        "artifact": "BATCH08_STATUS_QUEUES",
        "generated_at": datetime.now(UTC).isoformat(),
        "git_commit": baseline_head,
...
def validate_package(docs: dict[str, dict[str, Any]]) -> None:
    ...
    dup = docs["BATCH08_DUPLICATE_CANONICAL_ANALYSIS.json"]
    ...
    collective = docs["BATCH08_COLLECTIVE_REVIEW_LOCAL.json"]
    assert collective["summary"]["collective_review_local_complete"] == EXPECTED_COUNT

    perf = docs["BATCH08_PERFORMANCE_CAPACITY_PREP.json"]
    ...
    v3state = docs["BATCH08_V3_STATE_CLASSIFICATION.json"]
    ...
    assert dup_ex.get("evaluated_cross_batch_pairs") == 17500
    assert dup_ex.get("duplicate_coverage_complete") is True

    hero_m = docs["BATCH08_HERO_MATRIX_351_400.json"]
    assert hero_m["summary"]["hero_matrix_cells"] == 300
    assert hero_m["summary"]["duplicate_hero_contributions"] == 0

    fpe = docs["BATCH08_FULL_PATH_ENTITLEMENT.json"]
    assert fpe["passed"] is True

    freeze = docs["BATCH08_FINAL_LOCAL_FREEZE.json"]
    assert freeze["freeze_assertions"]["BATCH08_FINAL_LOCAL_FREEZE"] is True

    queues = docs["BATCH08_STATUS_QUEUES.json"]
    ...
    matrix = docs["BATCH08_PER_ID_FINAL_MATRIX_351_400.json"]
    assert matrix["summary"]["pass_engineering"] == EXPECTED_COUNT

    heroes = docs["BATCH08_SIX_HEROES_BINDING.json"]
...
def fetch_ci_evidence_for_head(head: str, branch: str = "cursor/batch08-351-400-ed16") -> dict[str, Any]:
...
def main() -> None:
    ...
    docs: dict[str, dict[str, Any]] = {
        "BATCH08_BASELINE.json": build_baseline(bindings, catalog, audit, baseline_head, tested_head),
        "BATCH08_DUPLICATE_CANONICAL_ANALYSIS.json": duplicate_doc,
        "BATCH08_EXISTING_VERIFIED_EVIDENCE.json": evidence_doc,
        "BATCH08_COLLECTIVE_REVIEW_LOCAL.json": collective_doc,
        "BATCH08_RTM_351_400.json": build_rtm(bindings, catalog, audit_by, baseline_head),
        "BATCH08_PER_ID_FINAL_MATRIX_351_400.json": build_per_id_matrix(
            bindings, catalog, audit_by, baseline_head
        ),
        "BATCH08_PENTAGONAL_TEMPLATE_351_400.json": build_pentagonal_template(
            bindings, catalog, audit_by, baseline_head
        ),
        "BATCH08_PENTAGONAL_COL10_PREPARATION.json": build_col10_preparation(baseline_head),
        "BATCH08_DATA_QUALITY_INTEGRITY.json": build_data_quality(audit, baseline_head),
        "BATCH08_SECURITY_MATERIAL_PATH_AUDIT.json": build_security_audit(baseline_head),
        "BATCH08_RELIABILITY_FAILURE_MODES.json": build_reliability(baseline_head),
        "BATCH08_OBSERVABILITY_READINESS.json": build_observability(baseline_head),
        "BATCH08_PERFORMANCE_CAPACITY_PREP.json": build_performance_prep(
            baseline_head, perf_benchmark, full_path_perf
        ),
        "BATCH08_SIX_HEROES_BINDING.json": build_six_heroes(catalog, baseline_head),
        "BATCH08_V3_STATE_CLASSIFICATION.json": v3_state,
        "BATCH08_FULL_PATH_ENTITLEMENT.json": full_path_ent,
        "BATCH08_FULL_PATH_PERFORMANCE.json": {**full_path_perf, "git_commit": baseline_head},
        "BATCH08_HERO_MATRIX_351_400.json": hero_matrix,
        "BATCH08_EVENT_LOOP_FORENSIC.json": event_loop,
        "BATCH08_SSOT_RECONCILIATION.json": ssot,
        "BATCH08_12207_VALIDATION_PACKAGE.json": build_validation_package(
            catalog, audit_by, baseline_head
        ),
        "BATCH08_12207_TRANSITION_PACKAGE.json": build_transition_package(baseline_head),
        "BATCH08_12207_OPERATION_READINESS_PACKAGE.json": build_operation_package(baseline_head),
        "BATCH08_SRE_PRR_PACKAGE.json": build_sre_prr(baseline_head),
        "BATCH08_G7_PRE_ASSURANCE_PACKAGE.json": build_g7_package(baseline_head),
    }

    regression = build_cross_batch_regression(tested_head)
    pip_audit = run_pip_audit()
    docs["BATCH08_CROSS_BATCH_REGRESSION.json"] = regression
    docs["BATCH08_STATUS_QUEUES.json"] = status_queues
    docs["BATCH08_FINAL_LOCAL_FREEZE.json"] = build_final_freeze(
        baseline_head,
        regression,
        pip_audit,
        tested_head,
    ...
    print(f"Wrote {len(written)} Batch08 institutional artifacts @ {baseline_head[:12]}")
    for name in written:
        print(f"  docs/{name}")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "--ci-freeze-only":
        ...
        regression = docs["BATCH08_CROSS_BATCH_REGRESSION.json"]
        pip_audit = run_pip_audit()
        ...
        full_path_perf = docs["BATCH08_FULL_PATH_PERFORMANCE.json"]
        freeze = build_final_freeze(
            ...
            drift=drift,
            duplicate_doc=duplicate_doc,
            v3_state=docs["BATCH08_V3_STATE_CLASSIFICATION.json"],
            full_path_ent=docs["BATCH08_FULL_PATH_ENTITLEMENT.json"],
