...
OUT = ROOT / "bd_platform/institutional_delivery_intelligence_layer.py"
...
def main() -> None:
    ...
    for cid in range(501, 601):
        ...
        key = f"cap_{cid}"
        if key not in seed:
            seed[key] = {"metric": round(default + (cid - 501) * 3.7, 4)}
            changed_seed = True
        body.append(
            f'''
def {fn}(*, symbol: str = "BTC", seed: dict[str, Any] | None = None) -> dict[str, Any]:
    """{title} (#{cid})."""
    seed = seed or _load_seed()
    metric = _metric(seed, "{key}", {round(default + (cid - 501) * 3.7, 4)})
    return _base(
        {cid},
        symbol=symbol,
        seed=seed,
