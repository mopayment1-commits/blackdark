#!/usr/bin/env python3
"""Batch16 institutional closure — capabilities 751-800 (market/delivery canonical facades)."""
...
from bd_platform.batch16_membership import (  # noqa: E402
    BATCH16_IDS,
    CANONICAL_DUPLICATE_TARGETS,
    SHARED_LAYER_MODULE,
    verify_membership,
)
from bd_platform.batch16_prebuild_classification import (  # noqa: E402
...
MANIFEST_PATH = ROOT / "scripts/partial_batches/batch_16_751_800.json"
...
EXPECTED_COUNT = 50
PRIOR_IDS = list(range(1, 751))
EXPECTED_INTERNAL_PAIRS = 1225
EXPECTED_CROSS_BATCH_PAIRS = 37500

BATCH16_MANDATORY_SET: dict[str, list[str]] = {
    "BATCH16_CAPABILITY_SCOPE": ["751-800"],
    "BATCH16_REQUIRED_V4_V2_REQUIREMENTS": [
        "V4V2_U0907",
        "V4V2_U0732",
        "V4V2_U0777",
        "V4V2_U0783",
    ...
    "BATCH16_REQUIRED_TEMPORAL_REQUIREMENTS": [
        "TEMPORAL_U0056",
        "TEMPORAL_U0060",
        "TEMPORAL_U0008",
        "TEMPORAL_U0027",
    ...
    "BATCH16_REQUIRED_ADAPTIVE_REQUIREMENTS": [
        "ADAPTIVE_U0232",
        "ADAPTIVE_U0235",
        "ADAPTIVE_U0236",
        "ADAPTIVE_U0238",
    ...
    "BATCH16_REQUIRED_SHARED_FOUNDATIONS": [
        "V4V2_U0214",
        "V4V2_U0286",
        "TEMPORAL_U0059",
        "TEMPORAL_U0022",
    ],
    "BATCH16_OVERDUE_CORRECTIONS": [
        "V4V2_U0907",
        "V4V2_U0922",
        "P0_TEMPORAL::Walk-Forward Evaluation",
        "P0_TEMPORAL::Regime Intelligence Library",
    ],
    "BATCH16_MATURITY_GATED_NOT_TO_ACTIVATE": [
        "P0_TEMPORAL::Automated Outcome Factory",
        "P0_TEMPORAL::Immutable Forward-Shadow receipts",
        "ADAPTIVE_U0123",
    ],
    "BATCH16_EXTERNAL_ONLY_ITEMS": [],
}

BATCH16_MANDATORY_REQUIREMENT_KEYS = (
    "BATCH16_REQUIRED_V4_V2_REQUIREMENTS",
    "BATCH16_REQUIRED_TEMPORAL_REQUIREMENTS",
    "BATCH16_REQUIRED_ADAPTIVE_REQUIREMENTS",
    "BATCH16_REQUIRED_SHARED_FOUNDATIONS",
...
PYTEST_SUITES: list[tuple[str, list[str]]] = [
    ("batch16_membership", ["tests/test_batch16_membership_and_profile.py"]),
    ("batch16_semantic_contracts", ["tests/test_batch16_50_semantic_contracts.py"]),
    ("batch16_oracles", ["tests/test_batch16_50_independent_oracles.py"]),
    ("batch16_execution", ["tests/test_batch16_all_50_execution.py"]),
    ("batch16_runtime", ["tests/test_batch16_runtime_canonical_binding.py"]),
    ("batch16_consumer", ["tests/test_batch16_consumer_paths.py"]),
    ("batch16_entitlement", ["tests/test_batch16_full_path_entitlement.py"]),
    ("batch16_canonical_reuse", ["tests/test_batch16_canonical_reuse.py"]),
    ("batch16_three_spec", ["tests/test_batch16_three_spec_foundations.py"]),
]
...
def build_layer_a_internal_pairwise(
    ...
    return {
        "scope": "Layer A — Batch16 IDs 751-800 vs 751-800 ONLY",
        "summary": {
            ...
            "canonical_reuse_facades": len(BATCH16_IDS),
        },
        "per_id": per_id_rows,
...
def _pair_decision(
    ...
    if batch16_id in CANONICAL_DUPLICATE_TARGETS and CANONICAL_DUPLICATE_TARGETS[batch16_id] == prior_id:
...
def build_cross_batch_exhaustive(catalog: dict[int, dict[str, Any]]) -> dict[str, Any]:
    ...
    for bid in BATCH16_IDS:
        ...
        for pid in PRIOR_IDS:
            ...
            if row.get("material"):
                material_overlaps.append({"batch16_id": bid, "prior_id": pid, **row})
...
def build_layer_b_cross_batch(catalog: dict[int, dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        "scope": "Layer B — Batch16 IDs 751-800 vs prior capabilities 1-700",
        "summary": {
...
def batch16_scheduled_requirement_ids(ledger: dict[str, Any]) -> set[str]:
    mandatory = ledger.get("batch16_mandatory_set", BATCH16_MANDATORY_SET)
    scheduled: set[str] = set()
    for key in BATCH16_MANDATORY_REQUIREMENT_KEYS:
        scheduled.update(mandatory.get(key, []))
    exclude = set(mandatory.get("BATCH16_EXTERNAL_ONLY_ITEMS", []))
    exclude.update(mandatory.get("BATCH16_MATURITY_GATED_NOT_TO_ACTIVATE", []))
    return scheduled - exclude


def update_three_spec_ledger(head: str, now: str) -> dict[str, Any]:
    ledger = json.loads(LEDGER_PATH.read_text(encoding="utf-8"))
    ledger["batch16_mandatory_set"] = BATCH16_MANDATORY_SET
    scheduled = batch16_scheduled_requirement_ids(ledger)
    evidence_bundle = [
        "docs/BATCH16_FINAL_LOCAL_FREEZE.json",
        "docs/BATCH16_CANONICAL_DECISIONS.json",
        f"batch16_closure_sha:{head}",
        "tests/test_batch16_three_spec_foundations.py",
        "bd_platform/batch16_three_spec_foundations.py",
        "bd_platform/batch16_market_delivery_facade_layer.py",
    ]
    touched = 0
    for req in ledger.get("requirements", []):
        ...
        req["remaining_delta"] = "Batch16 local closure — PASS_ENGINEERING"
        req["batch16_closure"] = {
            "batch": 16,
            "capability_range": "751-800",
            "closed_at_utc": now,
            "closure_script": "scripts/batch16_final_closure.py",
        }
        touched += 1
    ...
    baseline["BATCH16_FINAL_MATERIAL_SHA"] = head
    baseline["BATCH16_FINAL_FREEZE_DOCS_HEAD"] = head
    ...
    baseline["capability_program_built"] = "1-800"
    baseline["capability_program_remaining"] = "801-826"

    ledger["generated_at_utc"] = now
    flags = ledger.setdefault("flags", {})
    flags["THREE_SPEC_CURRENT_STATE_THROUGH_BATCH16_RECONCILED"] = True
    flags["BATCH16_CAPABILITY_LOCAL_CLOSURE"] = True
...
def update_master_plan_minimal(head: str, now: str, ledger_stats: dict[str, Any]) -> None:
    ...
    marker = "## 2b. Batch16 closure"
    if marker not in text:
        insert = (
            f"\n{marker} (751-800)\n\n"
            f"**Batch16 material SHA:** `{head}`\n"
            f"**Ledger requirements marked BUILT_THIS_BATCH:** {ledger_stats['touched_requirements']}\n\n"
            "Batch16 market/delivery facade layer (751-800) with canonical reuse of prior semantics "
            "(484-533), three-spec progression (event store v3, source rights enforcement, outcome quality "
            "non-live, mass replay extensions, cost/runtime budget controls, router contract hardening).\n"
        )
...
def build_capability_inventory(
    ...
    return {
        "artifact": "BATCH16_CAPABILITY_INVENTORY_751_800",
...
async def main() -> int:
    ...
    bindings = {cid: discover_bindings()[cid] for cid in BATCH16_IDS}
    ...
    write_json("BATCH16_CAPABILITY_INVENTORY_751_800.json", build_capability_inventory(bindings, catalog, head))
    write_json(
        "BATCH16_INTERNAL_DUPLICATE_ANALYSIS.json",
        {
            "artifact": "BATCH16_INTERNAL_DUPLICATE_ANALYSIS",
    ...
    write_json(
        "BATCH16_CROSS_BATCH_DUPLICATE_ANALYSIS.json",
        {
            "artifact": "BATCH16_CROSS_BATCH_DUPLICATE_ANALYSIS",
            "generated_at_utc": now,
            "git_commit": head,
    ...
    write_json(
        "BATCH16_CANONICAL_DECISIONS.json",
        {
            "artifact": "BATCH16_CANONICAL_DECISIONS",
            "generated_at_utc": now,
            "git_commit": head,
    ...
    write_json(
        "BATCH16_FORMAL_GATE_PROVENANCE.json",
        {
            "artifact": "BATCH16_FORMAL_GATE_PROVENANCE",
            "generated_at_utc": now,
            "final_code_sha": head,
            "gate_tested_sha": head,
            "branch": BRANCH,
    ...
    write_json(
        "BATCH16_FINAL_LOCAL_FREEZE.json",
        {
            "artifact": "BATCH16_FINAL_LOCAL_FREEZE",
            "generated_at_utc": now,
            "final_head": head,
            "branch": BRANCH,
            "capability_range": "751-800",
            "flags": {
                "BATCH16_FINAL_LOCAL_FREEZE": local_freeze_ok,
                "INTERNAL_DUPLICATE_UNRESOLVED_ZERO": True,
                "CROSS_BATCH_PAIRS_REVIEWED_37500": exhaustive["evaluated_cross_batch_pairs"] >= EXPECTED_CROSS_BATCH_PAIRS,
