...
Master Contract Run 027 — RBAS-001 risk-based scoping with Tier1 full 9-phase / Tier2 abbreviated."""
...
def cross_spine_preflight(cid: int) -> tuple[str, str | None]:
    ...
    if cid in CROSS_SPINE_RESOLVED_IDS:
        return "PASS", "Run 027 cross-spine resolved — batch08 spine only"
...
def _impact_metrics(rows: list[dict]) -> dict[str, Any]:
    ...
    return {
        ...
        "calibration_note": (
            "Escalation excludes compliance_footer provenance_score paths (Run 027 calibration). "
...
def write_report(rows: list[dict], tier_map: dict[int, dict[str, Any]]) -> Path:
    ...
    if not CONCEPTUAL_FLAGS:
        lines.append("- *(none flagged in Run 027 static pre-scan — live audit above is authoritative)*\n")
    lines.append("\n## رأي اللجنة المستقلة\n\n")
    lines.append(
