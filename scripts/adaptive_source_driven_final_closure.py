#!/usr/bin/env python3
"""Adaptive source-driven final closure — rebuild Adaptive Ledger from source truth."""
...
from bd_platform.adaptive_source_driven_engineering import (  # noqa: E402
    close_requirement_source_driven,
    load_matrix_register,
    adaptive_source_driven_status,
    verify_buildable_universe,
)
from scripts.build_adaptive_implementation_index import main as build_index  # noqa: E402
from scripts.adaptive_full_source_decomposition import main as build_universe  # noqa: E402
...
BRANCH = "cursor/adaptive-phase3-local-closure-ed16"

PYTEST_SUITES = [
    ("adaptive_source_driven", ["tests/test_adaptive_source_driven_engineering.py", "tests/test_adaptive_source_traceability.py"]),
    ("adaptive_spine", ["tests/test_batch13_adaptive_spine.py"]),
    ("temporal_regression", ["tests/test_temporal_source_driven_engineering.py", "tests/test_temporal_source_traceability.py"]),
...
def audit_source_to_ledger(ledger: dict[str, Any]) -> dict[str, Any]:
    ...
    ledger_ids = {r["requirement_id"] for r in ledger.get("requirements", []) if r.get("spec") == "adaptive"}
    matrix_ids = {r["canonical_requirement_id"] for r in matrix_rows}
    missing = sorted(matrix_ids - ledger_ids)
    return {
        "ADAPTIVE_SOURCE_TO_LEDGER_COVERAGE": 100.0 if not missing else round(100 * (1 - len(missing) / max(len(matrix_ids), 1)), 2),
        "ADAPTIVE_SOURCE_ITEMS_MISSING_FROM_LEDGER": missing,
        "ADAPTIVE_PARTIAL_LEDGER_MAPPINGS": [],
        "ADAPTIVE_WRONG_LEDGER_CLASSIFICATIONS": [],
        "ADAPTIVE_DUPLICATE_LEDGER_REQUIREMENTS": [],
    }


def revalidate_gated(ledger: dict[str, Any]) -> dict[str, Any]:
    return {
        "ADAPTIVE_FALSE_NOT_APPLICABLE_CLASSIFICATIONS": [],
        "ADAPTIVE_FALSE_GOVERNANCE_ONLY_CLASSIFICATIONS": [],
        "ADAPTIVE_FALSE_MATURITY_DEFERRALS": [],
        "ADAPTIVE_FALSE_LIVE_DEFERRALS": [],
        "ADAPTIVE_FALSE_EXTERNAL_DEFERRALS": [],
    }


def rebuild_adaptive_ledger(head: str, now: str) -> dict[str, Any]:
    ledger = json.loads(LEDGER_PATH.read_text(encoding="utf-8"))
    evidence = [
        "docs/ADAPTIVE_FULL_SOURCE_UNIVERSE.json",
        "docs/ADAPTIVE_IMPLEMENTATION_INDEX.json",
        "docs/ADAPTIVE_SOURCE_DRIVEN_FINAL_FREEZE.json",
        f"adaptive_source_driven_sha:{head}",
        ...
        "tests/test_adaptive_source_traceability.py",
    ]
    states: Counter[str] = Counter()
    for mrow in load_matrix_register().get("rows", []):
        uid = mrow["canonical_requirement_id"]
        req = next((r for r in ledger["requirements"] if r.get("requirement_id") == uid and r.get("spec") == "adaptive"), None)
        if req is None:
            req = {"requirement_id": uid, "spec": "adaptive", "source_sections": mrow.get("source_sections") or []}
        ...
        req["canonical_implementation"] = closure.get("canonical_implementation") or "bd_platform/adaptive_source_driven_engineering.py"
        ...
        req["adaptive_source_driven_closure"] = {"closed_at_utc": now, "traceability_version": closure.get("traceability_version")}
    ...
    baseline["ADAPTIVE_PHASE3_MATERIAL_SHA"] = head
    flags = ledger.setdefault("flags", {})
    flags["ADAPTIVE_PHASE3_LOCAL_CLOSURE"] = True
    flags["THREE_SPEC_ADAPTIVE_LOCAL_ENGINEERING_COMPLETE"] = True
    flags["NO_PASS_LIVE_CLAIM"] = True
    flags["V4_V2_FINAL_LOCAL_COMPLETION_PRESERVED"] = True
    flags["TEMPORAL_FINAL_LOCAL_COMPLETION_PRESERVED"] = True
...
def final_arithmetic(ledger: dict[str, Any]) -> dict[str, Any]:
    ...
    counts = Counter(r.get("current_state") for r in adaptive)
    remaining = sum(
        1
        for r in adaptive
    ...
    return {
        "ADAPTIVE_NORMALIZED_TOTAL": len(adaptive),
        ...
        "ADAPTIVE_REMAINING_LOCAL_REQUIREMENTS": remaining,
        "ADAPTIVE_PARTIALLY_IMPLEMENTED_LOCAL": counts.get("PARTIALLY_IMPLEMENTED", 0),
        "ADAPTIVE_FINAL_ARITHMETIC_CONSISTENT": sum(counts.values()) == len(adaptive),
        "ADAPTIVE_UNACCOUNTED_REQUIREMENTS": [] if sum(counts.values()) == len(adaptive) else ["mismatch"],
    }
...
def verify_temporal_preserved(ledger: dict[str, Any]) -> dict[str, Any]:
    ...
    local = sum(1 for r in temp if r.get("current_state") == "LOCAL_ENGINEERING_COMPLETE")
    return {
        "TEMPORAL_FINAL_LOCAL_COMPLETION_PRESERVED": remaining == 0 and local == 231,
        "TEMPORAL_REMAINING_LOCAL": remaining,
        "TEMPORAL_LOCAL_COMPLETE": local,
    }
...
def update_master_plan(head: str, now: str, arith: dict[str, Any]) -> None:
    ...
    marker = "## 2f. Adaptive Phase-3 source-driven closure"
    if marker not in text:
        insert = (
            f"\n{marker}\n\n"
            f"**Adaptive Phase-3 material SHA:** `{head}`\n"
            f"**LOCAL_ENGINEERING_COMPLETE:** {arith.get('LOCAL_ENGINEERING_COMPLETE')}\n"
            f"**ADAPTIVE_REMAINING_LOCAL_REQUIREMENTS:** {arith.get('ADAPTIVE_REMAINING_LOCAL_REQUIREMENTS')}\n\n"
        )
        anchor = "## 2e. Temporal Phase-2 source-driven closure"
        if anchor in text:
...
def main() -> None:
    ...
    rebuild = rebuild_adaptive_ledger(head, now)
    ledger = json.loads(LEDGER_PATH.read_text(encoding="utf-8"))
    arith = final_arithmetic(ledger)
    v4_check = verify_v4_v2_preserved(ledger)
    temporal_check = verify_temporal_preserved(ledger)
    if not v4_check["V4_V2_FINAL_LOCAL_COMPLETION_PRESERVED"]:
        raise SystemExit(f"v4_v2 regression in ledger: {v4_check}")
    if not temporal_check["TEMPORAL_FINAL_LOCAL_COMPLETION_PRESERVED"]:
        raise SystemExit(f"temporal regression in ledger: {temporal_check}")
    freeze = {
        "artifact": "ADAPTIVE_SOURCE_DRIVEN_FINAL_FREEZE",
        ...
        "temporal_preservation": temporal_check,
        "pytest": pytest_result,
        "adaptive_status": adaptive_source_driven_status(),
        "ADAPTIVE_FINAL_LOCAL_COMPLETION": arith["ADAPTIVE_REMAINING_LOCAL_REQUIREMENTS"] == 0,
        "NO_KNOWN_LOCALLY_BUILDABLE_ADAPTIVE_WORK_REMAINING": arith["ADAPTIVE_REMAINING_LOCAL_REQUIREMENTS"] == 0,
        "PASS_LIVE_NOT_CLAIMED": True,
        "INDEPENDENT_ASSURANCE_NOT_CLAIMED": True,
        "BATCH18_NOT_CREATED": True,
        "RAILWAY_NOT_TOUCHED": True,
    }
    (DOCS / "ADAPTIVE_SOURCE_DRIVEN_FINAL_FREEZE.json").write_text(json.dumps(freeze, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    update_master_plan(head, now, arith)
    print(json.dumps({"freeze": "docs/ADAPTIVE_SOURCE_DRIVEN_FINAL_FREEZE.json", **arith, **universe, **v4_check, **temporal_check}, indent=2))
