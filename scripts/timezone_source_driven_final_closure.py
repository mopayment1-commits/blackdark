#!/usr/bin/env python3
"""Timezone source-driven final closure."""
...
def main() -> int:
    subprocess.run([sys.executable, str(ROOT / "scripts/build_timezone_implementation_index.py")], check=True)
    proc = subprocess.run(
        [
            sys.executable,
            "-m",
            "pytest",
            "tests/test_timezone_p0_test_matrix.py",
    ...
    status = timezone_source_driven_status(head=head, pytest_ok=proc.returncode == 0)
    ledger = {
        "schema_version": "1.0",
    ...
    ledger_path = ROOT / "docs" / "TIMEZONE_IMPLEMENTATION_LEDGER.json"
    ledger_path.write_text(json.dumps(ledger, indent=2) + "\n", encoding="utf-8")
    ledger["ledger_sha256"] = sha256_file(ledger_path)
    closure_path = ROOT / "docs" / "TIMEZONE_36_CLOSURE.json"
    ...
    print(json.dumps({k: artifact[k] for k in sorted(artifact) if k.startswith(("PASS_", "SOURCE_", "LOCAL_", "KNOWN_", "CANONICAL_", "P0_"))}, indent=2))
    return 0 if artifact["PASS_ENGINEERING_TIMEZONE"] else 1
