#!/usr/bin/env python3
"""Master Contract Run 025 — Batch 05 final closure (IDs 251–300)."""
...
RTM_PATH = ROOT / "docs" / "BATCH05_OFFICIAL_RTM_251_300.json"
...
async def score_idx_supplemental_review(not_complete_ids: list[int]) -> dict[str, Any]:
    ...
    return {
        ...
        "policy": "SCORE-IDX-001 + RBAS-001 Tier2 non-escalated recheck (Run 025)",
...
async def bcbs_remediation_evidence(not_complete_ids: list[int]) -> dict[str, Any]:
    ...
    evidence: dict[str, Any] = {
        ...
        "fix": "cap646.batch05_dedicated._wrap stamps top-level data_source + timestamp (Run 024; verified Run 025)",
        "per_id": {},
    }
...
def write_rtm(rows: list[dict], id277: dict[str, Any]) -> dict[str, Any]:
    ...
    for r in rows:
        ...
        entry: dict[str, Any] = {
            ...
            "audit_method": "Independent Third-Line RBAS (Run 025 post-closure)",
    ...
    doc = {
        "generated_at": datetime.now(UTC).isoformat(),
        "scope": "Official Batch 05 — IDs 251–300",
        "supersedes": "Run 024 diagnostic RBAS audit (0 PRODUCTION-ALIGNED — honest)",
        "audit_method": "Independent Third-Line RBAS Due Diligence — Run 025 closure",
        "summary": {
            "total": 50,
...
def write_final_closure_report(
    ...
    lines = [
        "# Batch 05 Final Closure Report (Run 025)\n\n",
        ...
        f"| RTM updated (0 PRODUCTION-ALIGNED honest) | **MET ✅** (`docs/BATCH05_OFFICIAL_RTM_251_300.json`) |\n",
    ...
    lines.extend(
        [
            ...
            "- `docs/BATCH05_OFFICIAL_RTM_251_300.json` — honest RTM (0 PRODUCTION-ALIGNED)\n",
    ...
    lines.append("\n## رأي اللجنة المستقلة\n\n")
    lines.append(
        f"بعد Run 025، Batch 05 (251–300) **مغلق رسميًا**. CONCEPTUALLY-UNSOUND=0 (29 Tier2 non-escalated recheck). "
    ...
    (OUT / "RUN016_BATCH05_CLOSURE_EVIDENCE.json").write_text(
        json.dumps(
            {
