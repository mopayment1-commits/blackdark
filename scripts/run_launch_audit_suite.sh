...
status_set() { echo "$2" >"$OUT/$1.status"; }
...
# A — dependencies
run pip_audit $PY -m pip_audit -r requirements.lock.txt --desc
...
if command -v gitleaks >/dev/null 2>&1; then
  ...
  echo "SKIP gitleaks"; status_set gitleaks SKIP
fi
...
if command -v trivy >/dev/null 2>&1; then
  ...
  echo "SKIP trivy"; status_set trivy_fs SKIP
fi
if command -v hadolint >/dev/null 2>&1; then
  run hadolint hadolint Dockerfile
else
  echo "SKIP hadolint"; status_set hadolint SKIP
fi
...
if [[ "${RUN_ZAP:-0}" == "1" && -n "${TARGET_URL:-}" ]] && command -v docker >/dev/null 2>&1; then
  ...
  echo "SKIP zap_baseline (set RUN_ZAP=1 TARGET_URL=...)"; status_set zap_baseline SKIP
fi
...
if [[ "${RUN_LIGHTHOUSE:-0}" == "1" && -n "${TARGET_URL:-}" ]] && command -v npx >/dev/null 2>&1; then
  ...
  echo "SKIP lighthouse (set RUN_LIGHTHOUSE=1 TARGET_URL=...)"; status_set lighthouse SKIP
fi
