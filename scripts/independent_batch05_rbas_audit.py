#!/usr/bin/env python3
"""Independent Third-Line RBAS Due Diligence — Official Batch 05 (IDs 251–300).
Master Contract Run 024 — RBAS-001 risk-based scoping with Tier1 full 9-phase / Tier2 abbreviated."""
...
def batch05_tier_map():
    return batch_tier_map(5)


WF027_IN_BATCH05 = WF027_UNRESOLVED_LEGACY_IDS & set(range(201, 251))
...
AI_CAP_IDS = discover_ai_cap_ids(5)
...
async def split_brain_test(cid: int) -> dict[str, Any]:
    ...
    from cap646.batch05_dedicated import BATCH05_DEDICATED_IDS, execute as execute_b6d
...
def cross_spine_preflight(cid: int) -> tuple[str, str | None]:
    ...
    if cid in CROSS_SPINE_RESOLVED_IDS:
        return "PASS", "Run 024 cross-spine resolved — batch05 spine only"
...
def _impact_metrics(rows: list[dict]) -> dict[str, Any]:
    ...
    return {
        ...
        "calibration_note": (
            "Escalation excludes compliance_footer provenance_score paths (Run 024 calibration). "
...
def write_report(rows: list[dict], tier_map: dict[int, dict[str, Any]]) -> Path:
    ...
    lines.extend(
        [
            ...
            f"- **WF-027 dormant legacy in batch05 range (251–300):** `{sorted(WF027_IN_BATCH05)}` "
    ...
    if not CONCEPTUAL_FLAGS:
        lines.append("- *(none flagged in Run 024 static pre-scan — live audit above is authoritative)*\n")
    lines.append("\n## رأي اللجنة المستقلة\n\n")
    lines.append(
        f"بصفتنا لجنة تدقيق مستقلة (Third Line of Defense — IIA IPPF)، وبعد تنفيذ RBAS-001 "
        f"على Batch 05 (IDs 251–300) وفق SR 26-2 وCOSO وGIPS وRTM-IND-001 وCROSS-SPINE-001، "
