#!/usr/bin/env python3
"""Independent Third-Line RBAS Due Diligence — Official Batch 05 (IDs 201–250).
Master Contract Run 013 — RBAS-001 risk-based scoping with Tier1 full 9-phase / Tier2 abbreviated."""
...
CROSS_SPINE_RESOLVED_IDS = frozenset({214, 245})

DECISION_CAP_IDS = {223, 224, 225, 226, 227, 229, 230, 237, 238, 240, 242}
AI_CAP_IDS = {224, 226, 227, 238, 241, 242}
...
COMMON_PARAMS = {
    ...
    "email": "run013-audit@blackdark.local",
    "tier": "pro",
}
...
BATCH05_SPINES = frozenset({"batch05_prep", "batch05"})
...
async def split_brain_test(cid: int) -> dict[str, Any]:
    ...
    from cap646.batch05_dedicated import BATCH05_DEDICATED_IDS, execute as execute_b5d
    ...
    if cid in BATCH05_DEDICATED_IDS:
        try:
            dedicated = await execute_b5d(cid, params=params)
            row["batch05_dedicated_available"] = True
            row["dedicated_result"] = dedicated
        except Exception as exc:
            row["batch05_dedicated_available"] = False
            row["dedicated_error"] = f"{type(exc).__name__}: {exc}"
    else:
        row["batch05_dedicated_available"] = False
        row["dedicated_error"] = "not_in_BATCH05_DEDICATED_IDS"
    ...
    elif not row.get("free_tier_available") and row.get("batch05_dedicated_available"):
        row["result_type"] = "DEDICATED_ONLY"
        row["verdict"] = "No free_tier path — dedicated batch05 only (SPLIT-BRAIN N/A)"
    elif row.get("free_tier_available") and row.get("batch05_dedicated_available"):
...
async def load_split_brain() -> dict[int, dict]:
    ...
    if _split_cache is not None:
        return _split_cache
    rows = [await split_brain_test(cid) for cid in BATCH05_RANGE]
    _split_cache = {r["id"]: r for r in rows}
    (OUT / "RUN013_SPLIT_BRAIN_EVIDENCE.json").write_text(
...
def backend_source(cid: int) -> tuple[str, list[str]]:
    ...
    mod = ROOT / "cap646" / "batch05_dedicated.py"
    ...
    if cid in BATCH05_DEDICATED_IDS:
        return "cap646.batch05_dedicated", lines_out or [f"_cap{cid:03d} handler"]
    return "cap646.batch05_production unmapped", [f"capability {cid} not in BATCH05_DEDICATED_IDS"]
...
def cross_spine_preflight(cid: int) -> tuple[str, str | None]:
    ...
    if cid in CROSS_SPINE_RESOLVED_IDS:
        return "PASS", "Run 013 cross-spine resolved — batch05 spine only"
...
def phase1_conceptual(cid: int, result: dict, *, split: dict) -> tuple[str, str | None]:
    ...
    if result.get("production_spine") not in BATCH05_SPINES:
        return "FAIL", f"CROSS_SPINE: official batch05 but production_spine={result.get('production_spine')}"
...
def phase7_iso(cid: int, result: dict) -> tuple[str, str | None]:
    ...
    if result.get("production_spine") not in BATCH05_SPINES:
        return "PARTIAL", f"traceability split: official batch05, spine={result.get('production_spine')}"
    return "PASS", None
...
def final_status_tier1(
    ...
    if spine not in BATCH05_SPINES:
...
def _build_evidence(
    ...
    if cid in CROSS_SPINE_RESOLVED_IDS:
        evidence_parts.append(
            f"CROSS-SPINE-001 resolved Run013: spine={runtime.get('production_spine')} "
...
async def audit_all() -> list[dict]:
    ...
    tier_map = batch05_tier_map()
    (OUT / "RBAS001_TIER_CLASSIFICATION.json").write_text(
        json.dumps([tier_map[cid] for cid in BATCH05_RANGE], indent=2, ensure_ascii=False) + "\n",
        encoding="utf-8",
    )
...
def _impact_metrics(rows: list[dict]) -> dict[str, Any]:
    ...
    return {
        ...
        "calibration_note": (
            "Escalation excludes compliance_footer provenance_score paths (Run 013 calibration). "
...
def write_report(rows: list[dict], tier_map: dict[int, dict[str, Any]]) -> Path:
    md = OUT / "BATCH05_INDEPENDENT_RBAS_AUDIT_REPORT.md"
    ...
    lines.extend(
        [
            ...
            f"- **Routing overlaps (BATCH01∩BATCH02∩BATCH03∩BATCH05):** "
            f"{len(overlaps)} IDs `{sorted(overlaps.keys())}`\n",
            f"- **WF-027 dormant legacy in batch05 range (201–250):** `{sorted(WF027_IN_BATCH05)}`\n",
            f"- **Cross-spine resolved IDs 214/245:** dedicated batch05 handler Run 013; "
    ...
    if not CONCEPTUAL_FLAGS:
        lines.append("- *(none flagged in Run 013 static pre-scan — live audit above is authoritative)*\n")
    lines.append("\n## رأي اللجنة المستقلة\n\n")
    lines.append(
        f"بصفتنا لجنة تدقيق مستقلة (Third Line of Defense — IIA IPPF)، وبعد تنفيذ RBAS-001 "
        f"على Batch 05 (IDs 201–250) وفق SR 26-2 وCOSO وGIPS وRTM-IND-001 وCROSS-SPINE-001، "
        ...
        f"**Batch 05 غير مغلق** — بوابة الإغلاق: CONCEPTUALLY-UNSOUND=0 + RTM صادق. "
        ...
        f"IDs 214/245: WF-027/CROSS-SPINE-001 resolved Run 013 — handler batch05_dedicated، "
        f"spine=batch05_prep. **نوصي بعدم أي إصلاح قبل مراجعة هذا التقرير** — "
        f"الإغلاق في Run منفصل كما Batch 01/02/03.\n"
    )
    md.write_text("".join(lines), encoding="utf-8")
    (OUT / "BATCH05_INDEPENDENT_RBAS_AUDIT.json").write_text(
...
async def main() -> None:
    ...
    counts = Counter(r["status"] for r in rows)
    metrics = _impact_metrics(rows)
    (OUT / "RUN013_RBAS_IMPACT_METRICS.json").write_text(
