#!/usr/bin/env python3
"""Independent Third-Line RBAS Due Diligence — Official Batch 14 (IDs 251–300).
...
def batch14_tier_map():
    return batch_tier_map(14)


WF027_IN_BATCH14 = WF027_UNRESOLVED_LEGACY_IDS & set(range(651, 701))

OUT = ROOT / "institutional_due_diligence_2026" / "batch14_independent_audit"
OUT.mkdir(parents=True, exist_ok=True)

BATCH14_RANGE = range(651, 701)
BATCH_NUM = 14
...
AI_CAP_IDS = discover_ai_cap_ids(14)
...
BATCH14_SPINES = frozenset({"batch14_prep", "batch14"})
...
async def split_brain_test(cid: int) -> dict[str, Any]:
    ...
    from cap646.batch14_dedicated import BATCH14_DEDICATED_IDS, execute as execute_b6d
    ...
    if cid in BATCH14_DEDICATED_IDS:
        try:
            dedicated = await execute_b6d(cid, params=params)
            row["batch14_dedicated_available"] = True
            row["dedicated_result"] = dedicated
        except Exception as exc:
            row["batch14_dedicated_available"] = False
            row["dedicated_error"] = f"{type(exc).__name__}: {exc}"
    else:
        row["batch14_dedicated_available"] = False
        row["dedicated_error"] = "not_in_BATCH14_DEDICATED_IDS"
    ...
    elif not row.get("free_tier_available") and row.get("batch14_dedicated_available"):
        row["result_type"] = "DEDICATED_ONLY"
        row["verdict"] = "No free_tier path — dedicated batch14 only (SPLIT-BRAIN N/A)"
    elif row.get("free_tier_available") and row.get("batch14_dedicated_available"):
        f_data = (row.get("free_result") or {}).get("data") or row.get("free_result")
        d_raw = row.get("dedicated_result") or {}
        d_data = d_raw.get("data")
        if d_data is None:
            from cap646.batch14_dedicated import EXPECTED_SURFACE
...
async def load_split_brain() -> dict[int, dict]:
    ...
    if _split_cache is not None:
        return _split_cache
    rows = [await split_brain_test(cid) for cid in BATCH14_RANGE]
...
def backend_source(cid: int) -> tuple[str, list[str]]:
    from cap646.batch14_dedicated import BATCH14_DEDICATED_IDS

    mod = ROOT / "cap646" / "batch14_dedicated.py"
    ...
    if cid in BATCH14_DEDICATED_IDS:
        return "cap646.batch14_dedicated", lines_out or [f"_cap{cid:03d} handler"]
    return "cap646.batch14_production unmapped", [f"capability {cid} not in BATCH14_DEDICATED_IDS"]
...
def cross_spine_preflight(cid: int) -> tuple[str, str | None]:
    ...
    if cid in CROSS_SPINE_RESOLVED_IDS:
        return "PASS", "Run 025 cross-spine resolved — batch14 spine only"
...
def phase1_conceptual(cid: int, result: dict, *, split: dict) -> tuple[str, str | None]:
    ...
    if result.get("production_spine") not in BATCH14_SPINES:
        return "FAIL", f"CROSS_SPINE: official batch14 but production_spine={result.get('production_spine')}"
...
def phase7_iso(cid: int, result: dict) -> tuple[str, str | None]:
    ...
    if result.get("production_spine") not in BATCH14_SPINES:
        return "PARTIAL", f"traceability split: official batch14, spine={result.get('production_spine')}"
    return "PASS", None
...
def final_status_tier1(
    ...
    if spine not in BATCH14_SPINES:
...
async def audit_all() -> list[dict]:
    ...
    tier_map = batch14_tier_map()
    (OUT / "RBAS001_TIER_CLASSIFICATION.json").write_text(
        json.dumps([tier_map[cid] for cid in BATCH14_RANGE], indent=2, ensure_ascii=False) + "\n",
        encoding="utf-8",
    )
...
def write_report(rows: list[dict], tier_map: dict[int, dict[str, Any]]) -> Path:
    md = OUT / "BATCH14_INDEPENDENT_RBAS_AUDIT_REPORT.md"
    ...
    lines.extend(
        [
            ...
            f"- **Routing overlaps (BATCH01∩BATCH02∩BATCH03∩BATCH14):** "
            f"{len(overlaps)} IDs `{sorted(overlaps.keys())}`\n",
            f"- **WF-027 dormant legacy in batch14 range (251–300):** `{sorted(WF027_IN_BATCH14)}` "
    ...
    lines.append(
        ...
        f"على Batch 14 (IDs 251–300) وفق SR 26-2 وCOSO وGIPS وRTM-IND-001 وCROSS-SPINE-001، "
        ...
        f"**Batch 14 غير مغلق** — بوابة الإغلاق: CONCEPTUALLY-UNSOUND=0 + RTM صادق. "
        ...
        f"spine=batch14_prep. **نوصي بعدم أي إصلاح قبل مراجعة هذا التقرير** — "
        f"الإغلاق في Run منفصل كما Batch 01/02/03.\n"
    )
    md.write_text("".join(lines), encoding="utf-8")
    (OUT / "BATCH14_INDEPENDENT_RBAS_AUDIT.json").write_text(
