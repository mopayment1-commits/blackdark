#!/usr/bin/env python3
"""Generate production launch secrets into a private gitignored file (mode 0600).

Secrets are never printed to stdout. Use --write (default) and open the file locally.
"""
...
from _secret_io import is_secret_env_key, mask_secret, write_private_text  # noqa: E402
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=True,
        help="Write secrets to .env.launch.local (gitignored, mode 0600) — default",
    )
    parser.add_argument(
        "--no-write",
        action="store_true",
        help="Do not write a file (dry-run summary only; secrets still not printed)",
    )
    parser.add_argument(
        "--app-base-url",
    ...
    print(f"\nWrote private file: {path} (mode 0600)")
    print("Open that file locally to copy values into Railway — do not paste secrets into chat/logs.")
