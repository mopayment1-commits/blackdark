#!/usr/bin/env python3
"""Batch 07 (301-350) deep closure: live exec, quad audit, REUSED-LINK tagging."""
...
BATCH_MANIFEST = ROOT / "scripts/partial_batches/batch_07_301_350.json"
EVIDENCE = ROOT / "data/hero_batch_07_301_350_evidence.jsonl"
GAP = ROOT / "docs/HERO_BATCH_07_301_350_GAP_REPORT.json"
AUDIT_JSON = ROOT / "docs/RETROSPECTIVE_DEEP_AUDIT_BATCH_07_301_350.json"
AUDIT_MD = ROOT / "docs/RETROSPECTIVE_DEEP_AUDIT_BATCH_07_REPORT.md"
SAMPLE_DOSSIER = ROOT / "docs/HERO_BATCH_07_SAMPLE_DOSSIER.json"


async def audit_batch07() -> dict:
...
def write_md(report: dict) -> None:
    ...
    lines = [
        "# Retrospective Deep Audit — Batch 07 (301–350)",
        "",
        f"**Audited at:** {report['audited_at']}",
        "",
        "## Honest Count (50 capabilities)",
        "",
...
def write_sample_dossier(report: dict) -> None:
    ...
    sample = random.Random(507301).sample(quad_pass, min(10, len(quad_pass)))
    dossier = {
        "batch": "batch_07_301_350",
        "sample_size": len(sample),
        "selection_seed": 507301,
        "capabilities": sample,
    }
...
async def main() -> int:
    ...
    if not args.skip_closure:
        print("Running batch 07 closure (live exec + gap report)...")
        summary = await run_closure("batch_07_301_350", upgrade=not args.skip_upgrade)
        print(
            json.dumps(
                {k: summary[k] for k in ("live_ok", "live_fail", "implemented_native", "delegated", "deferred")},
                indent=2,
            )
        )

    print("Running batch 07 deep quad audit...")
    report = await audit_batch07()
    ...
    _update_gap(GAP, report["rows"], id_set, "batch_07_301_350")
    _update_checklist(report["rows"])
    write_md(report)
    write_sample_dossier(report)

    c = report["classification_counts"]
    print("\n=== BATCH 07 HONEST COUNT ===")
    print(f"VERIFIED-DEEP (native):     {c['VERIFIED-DEEP']} / 50")
    print(f"REUSED-LINK:                {c['REUSED-LINK']} / 50")
    print(f"WRAPPER-ONLY-UNVERIFIED:    {c['WRAPPER-ONLY-UNVERIFIED']} / 50")
    print(f"DEFERRED/DELEGATED:         {c['DEFERRED/DELEGATED']} / 50")
    if c["WRAPPER-ONLY-UNVERIFIED"] > 0 or c["DEFERRED/DELEGATED"] > 0:
        print("BLOCKED: resolve non-quad classifications before claiming batch close")
        return 1
    return 0
