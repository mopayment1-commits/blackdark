#!/usr/bin/env python3
"""Generate RUNTIME_ENFORCEMENT_TABLE.md from REQUIREMENTS_REGISTER + honest live wiring map."""
...
REG = ROOT / "institutional_due_diligence_2026" / "DATA_STORAGE_TRACK_COMPLIANCE" / "REQUIREMENTS_REGISTER.json"
OUT = ROOT / "institutional_due_diligence_2026" / "DATA_STORAGE_TRACK_COMPLIANCE" / "RUNTIME_ENFORCEMENT_TABLE.md"

# req_id -> (status, live_code_evidence)
# status: YES | PARTIAL | NO — derived from actual call graph, not module existence alone
...
def main() -> None:
    ...
    for req in mandatory:
        rid = req["req_id"]
        status, evidence = RUNTIME_MAP.get(rid, ("NO", "NOT_WIRED"))
        if status == "YES":
            yes += 1
        elif status == "PARTIAL":
            partial += 1
            partial_items.append(rid)
        else:
            no += 1
            no_items.append(rid)
        req["runtime_enforced"] = status
        req["runtime_evidence"] = evidence
        lines.append(f"| {rid} | **{status}** | `{evidence}` |")

    lines.extend([
        "",
        f"**YES count:** {yes}/{len(mandatory)}",
        f"**PARTIAL count:** {partial}",
        f"**NO count:** {no}",
    ])
    if partial_items:
        lines.append(f"**Remaining PARTIAL:** {', '.join(partial_items)}")
    if no_items:
        lines.append(f"**Remaining NO:** {', '.join(no_items)}")

    OUT.write_text("\n".join(lines) + "\n", encoding="utf-8")
    data["runtime_enforcement"] = {
        "yes": yes,
        "partial": partial,
        "no": no,
        "total": len(mandatory),
    ...
    for req in data["requirements"]:
        rid = req["req_id"]
        if rid in RUNTIME_MAP:
            req["runtime_enforced"] = RUNTIME_MAP[rid][0]
            req["runtime_evidence"] = RUNTIME_MAP[rid][1]
    REG.write_text(json.dumps(data, indent=2, ensure_ascii=False), encoding="utf-8")
    print(f"Wrote {OUT} — YES={yes} PARTIAL={partial} NO={no} / {len(mandatory)}")
