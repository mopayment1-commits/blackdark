#!/usr/bin/env python3
"""Generate RUNTIME_ENFORCEMENT_TABLE.md from REQUIREMENTS_REGISTER + live wiring map."""
...
# req_id -> (enforced_in_runtime, live_code_evidence)
...
def main() -> None:
    ...
    lines.extend([
        ...
        f"**NO count:** {len(no_items)}",
    ])
    if no_items:
        lines.append(f"**Remaining NO:** {', '.join(no_items)}")

    OUT.write_text("\n".join(lines) + "\n", encoding="utf-8")
    data["runtime_enforcement"] = {"yes": yes, "no": len(no_items), "total": len(mandatory)}
    ...
    print(f"Wrote {OUT} — YES={yes}/{len(mandatory)}")
