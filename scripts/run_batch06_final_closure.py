#!/usr/bin/env python3
"""Master Contract Run 026 — Batch 06 final closure (IDs 251–300)."""
...
async def score_idx_supplemental_review(not_complete_ids: list[int]) -> dict[str, Any]:
    ...
    return {
        ...
        "policy": "SCORE-IDX-001 + RBAS-001 Tier2 non-escalated recheck (Run 026)",
...
async def bcbs_remediation_evidence(not_complete_ids: list[int]) -> dict[str, Any]:
    ...
    evidence: dict[str, Any] = {
        "generated_at": datetime.now(UTC).isoformat(),
        "fix": "cap646.batch06_dedicated._wrap stamps top-level data_source + timestamp (Run 025; verified Run 026)",
        "per_id": {},
    }
...
def write_rtm(rows: list[dict], id277: dict[str, Any]) -> dict[str, Any]:
    ...
    for r in rows:
        ...
        entry: dict[str, Any] = {
            ...
            "audit_method": "Independent Third-Line RBAS (Run 026 post-closure)",
    ...
    doc = {
        ...
        "supersedes": "Run 025 diagnostic RBAS audit (0 PRODUCTION-ALIGNED — honest)",
        "audit_method": "Independent Third-Line RBAS Due Diligence — Run 026 closure",
        "summary": {
            "total": 50,
...
def write_final_closure_report(
    ...
    lines = [
        "# Batch 06 Final Closure Report (Run 026)\n\n",
    ...
    lines.append("\n## رأي اللجنة المستقلة\n\n")
    lines.append(
        f"بعد Run 026، Batch 06 (251–300) **مغلق رسميًا**. CONCEPTUALLY-UNSOUND=0 (29 Tier2 non-escalated recheck). "
