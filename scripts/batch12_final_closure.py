#!/usr/bin/env python3
"""Batch12 institutional closure — capabilities 551-600 (institutional delivery entity intelligence shared core (551-600))."""
...
from bd_platform.batch12_membership import (  # noqa: E402
    BATCH12_IDS,
...
from bd_platform.batch12_semantic_engine import (  # noqa: E402
...
MANIFEST_PATH = ROOT / "scripts/partial_batches/batch_12_551_600.json"
AUDIT_PATH = DOCS / "RETROSPECTIVE_DEEP_AUDIT_BATCH_05_401_500.json"
BRANCH = "cursor/batch12-551-600-ed16"
EXPECTED_COUNT = 50
PRIOR_IDS = list(range(1, 551))
EXPECTED_INTERNAL_PAIRS = EXPECTED_COUNT * (EXPECTED_COUNT - 1) // 2  # 1225
EXPECTED_CROSS_BATCH_PAIRS = len(BATCH12_IDS) * len(PRIOR_IDS)  # 27500
...
CANONICAL_DECISIONS: dict[int, dict[str, Any]] = {
    551: {
        ...
        "facade_binding": "bd_platform.institutional_delivery_intelligence_layer.liquidation_intelligence_551",
        "canonical_implementation": "cap646.handlers.batch02.handle_batch02_capability → cap_088(88)",
        "underlying_module": "cap646.batch02_production",
        "underlying_function": "cap_088",
        "evidence": "Catalog duplicate Liquidation Intelligence — runtime delegates to canonical #88 batch02 liquidation_radar semantics; no parallel Batch12 semantic engine",
        "mece_action": "Count via canonical #88; #551 facade/pdf path must not add independent semantics",
    },
    578: {
        "decision": "OUTSIDE_SHARED_CORE_MODULE_BINDING",
        "binding_module": "bd_platform.institutional_delivery_intelligence_layer",
        "binding_function": "unified_portfolio_dashboard_578",
        "canonical_requirement": "Unified_Portfolio_Dashboard",
        "underlying_semantics": "bd_platform.portfolio_rebalancer.portfolio_snapshot",
        "evidence": "CAP646_CATALOG + RVM CAP-578 require Unified_Portfolio_Dashboard; hero shadow-fork binding was incorrect",
        "mece_action": "Outside shared core; catalog-aligned portfolio dashboard semantics",
        "hero_relation": "bd_platform.heroes_capability_layer.shadow_fork_pre_execution_578 is integration-only — does not define #578 identity",
    },
    584: {
        "decision": "OUTSIDE_SHARED_CORE_MODULE_BINDING",
        "binding_module": "bd_platform.institutional_delivery_intelligence_layer",
        "binding_function": "risk_management_shield_584",
        "canonical_requirement": "Risk_Management_Shield",
        "underlying_semantics": "risk_manager.risk_status",
        "evidence": "CAP646_CATALOG + RVM CAP-584 + gap_matrix risk_manager; CoinDesk hero binding was incorrect",
        "mece_action": "Outside shared core; catalog-aligned risk shield semantics",
        "hero_relation": "bd_platform.heroes_capability_layer.coindesk_rss_feed_584 is integration-only — does not define #584 identity",
    },
}
...
DOMAIN_SPEC_APPLICABILITY_DELTA: dict[str, dict[str, str]] = {
    ...
    "signal_registry": {
        ...
        "rationale": "Batch12 institutional delivery analytics are read-only intelligence — no signal emission registry per ID",
    },
    "prediction_ledger": {
        "applicability": "NOT_APPLICABLE",
...
TEMPORAL_CONTROLS_DELTA: dict[str, dict[str, str]] = {
    ...
    "deterministic_mass_replay_engine": {
        "applicability": "NOT_APPLICABLE",
        "rationale": "Batch12 surfaces are analytics payloads — no mass replay engine obligation per ID",
    },
...
SECURITY_CHECKS = [
    ...
    ("authorization", "PROVEN_LOCAL", "tests/test_batch12_consumer_paths.py execute probe"),
...
PYTEST_SUITES = [
    ("batch12_shared_core_semantics", ["tests/test_batch12_shared_core_semantics.py"]),
    ("batch12_independent_oracle_semantics", ["tests/test_batch12_independent_oracle_semantics.py"]),
    ("batch12_consumer_paths", ["tests/test_batch12_consumer_paths.py"]),
    ("batch12_runtime_canonical_binding", ["tests/test_batch12_runtime_canonical_binding.py"]),
    ("batch12_full_path_entitlement", ["tests/test_batch12_full_path_entitlement.py"]),
    ("batch12_cap551_reuse", ["tests/test_batch12_cap551_canonical_reuse.py"]),
    ("batch12_cap578_cap584_canonical", ["tests/test_batch12_cap578_cap584_canonical.py"]),
    ("batch12_membership", ["tests/test_batch12_membership_and_profile.py"]),
    ("institutional_delivery_501_600", ["tests/test_institutional_delivery_intelligence_batch501_600.py", "-q"]),
    ("batch11_cap550_blast_regression", ["tests/test_batch11_cap550_canonical_reuse.py"]),
...
def classify_id(cid: int) -> str:
    if cid == CANONICAL_DUPLICATE_REUSE_ID:
...
def build_layer_a_internal_pairwise(
    ...
    for cid in BATCH12_IDS:
        ...
        elif cid == CANONICAL_DUPLICATE_REUSE_ID:
        ...
        per_id_rows.append(
            {
                ...
                "note": "Layer A scope is intra-Batch12 only; #551 canonical duplicate is outside shared core",
            }
        )
    ...
    return {
        "scope": "Layer A — Batch12 IDs 551-600 vs 551-600 ONLY",
...
def _pair_decision(
    ...
    if batch12_id in CANONICAL_DECISIONS and CANONICAL_DECISIONS[batch12_id].get("canonical_capability_id") == prior_id:
        return {
            "decision": "CANONICAL_DUPLICATE_REUSE",
...
def build_cross_batch_exhaustive(
                ...
                material_overlaps.append(
                    {
                        ...
                        "batch12_binding": f"{b_bind[0]}.{b_bind[1]}",
    ...
    return {
        "scope": "Exhaustive machine coverage — Batch12 551-600 vs prior 1-500",
        "method": (
            "Deterministic pair evaluator over all 50×500 candidate pairs. "
            "No indexing filter may omit pairs — each (batch12_id, prior_id) receives a decision."
        ),
...
def build_layer_b_cross_batch(
    ...
    for cid in BATCH12_IDS:
        ...
        prior_matches = [
            ...
            if prior_id < 551 and prior_pair == target
        ]
        decision = "KEEP_DISTINCT_BUT_REUSE_SHARED_CORE"
        rows.append(
            {
                ...
                "evidence": f"pdf_capability_registry binding unique for batch12 #{cid}",
                "decision": decision,
    ...
    return {
        "scope": "Layer B — Batch12 IDs 551-600 vs prior capabilities 1-500",
        "summary": {
...
def neighbor_distinction(cap_id: int) -> str:
    ...
    if idx <= 0:
        return "First shared-core parameterized ID in batch12 institutional delivery layer"
    prev_id = peers[idx - 1]
        ...
        rows.append(
            {
                ...
                "independent_oracle_invariant": (
                    ...
                    "tests/test_batch12_independent_oracle_semantics.py"
                ),
                ...
                "shared_core_component": "bd_platform.institutional_delivery_intelligence_layer + batch12_semantic_engine",
                "exact_semantic_distinction_vs_neighbors": neighbor_distinction(cap_id),
                "classification": "KEEP_DISTINCT_BUT_REUSE_SHARED_CORE",
                "test_evidence_reference": "tests/test_batch12_shared_core_semantics.py",
                "tested_sha": head,
            }
        )
    return rows
        ...
        rows.append(
            {
                ...
                "oracle_source": "tests/batch12_independent_semantic_oracles.py",
                "production_source": "bd_platform.batch12_semantic_engine",
        ...
        rows.append(
            {
                ...
                "test_evidence": [
                    "tests/test_batch12_consumer_paths.py",
                    "tests/test_institutional_delivery_intelligence_batch551_600.py",
                ],
...
def build_capability_inventory(
    ...
    return {
        "artifact": "BATCH12_CAPABILITY_INVENTORY_551_600",
...
def build_rtm(
        ...
        rows.append(
            {
                ...
                "test": "tests/test_batch12_consumer_paths.py",
    ...
    return {
        "artifact": "BATCH12_RTM_551_600",
        "generated_at_utc": datetime.now(UTC).isoformat(),
        "git_commit": head,
        "scope": "Requirements traceability matrix — Batch12 551-600",
        "rows": rows,
...
def build_partial_reuse_analysis(head: str) -> dict[str, Any]:
    ...
    return {
        "artifact": "BATCH12_EXISTING_PARTIAL_REUSE_ANALYSIS",
        "generated_at_utc": datetime.now(UTC).isoformat(),
        "git_commit": head,
        "scope": "v4_v2 domain spec + Temporal Intelligence controls — Batch12 551-600",
...
def build_domain_spec_applicability(head: str) -> dict[str, Any]:
    ...
    return {
        "artifact": "BATCH12_DOMAIN_SPEC_APPLICABILITY",
        "generated_at_utc": datetime.now(UTC).isoformat(),
        "git_commit": head,
        "domain_spec_authority": str(DOMAIN_SPEC_PATH.relative_to(ROOT)),
        "scope": "Delta-only NOT_APPLICABLE / PARTIALLY_APPLICABLE mapping — institutional_delivery_entity_intelligence 551-600",
        "capability_type": DOMAIN_SPEC_CAPABILITY_TYPE,
...
def build_temporal_applicability(head: str) -> dict[str, Any]:
    ...
    return {
        "artifact": "BATCH12_TEMPORAL_INTELLIGENCE_APPLICABILITY",
        "generated_at_utc": datetime.now(UTC).isoformat(),
        "git_commit": head,
        "temporal_spec_authority": str(TEMPORAL_SPEC_PATH.relative_to(ROOT)),
        "scope": "Temporal Intelligence control delta mapping — Batch12 551-600",
...
def build_security_audit(head: str) -> dict[str, Any]:
    return {
        "artifact": "BATCH12_SECURITY_MATERIAL_PATH_AUDIT",
        "generated_at_utc": datetime.now(UTC).isoformat(),
        "git_commit": head,
        ...
        rows.append(
            {
                ...
                "status": "PROVEN_LOCAL",
                "evidence": "tests/test_batch12_consumer_paths.py",
            }
        )
    return {
        "artifact": "BATCH12_ENTITLEMENT_AUDIT",
        "generated_at_utc": datetime.now(UTC).isoformat(),
        "git_commit": head,
        "scope": "Entitlement and object-level authorization — Batch12 551-600",
...
def build_ssot_reconciliation(head: str) -> dict[str, Any]:
    return {
        "artifact": "BATCH12_SSOT_RECONCILIATION",
        "generated_at_utc": datetime.now(UTC).isoformat(),
        "git_commit": head,
...
async def main() -> int:
    ...
    bindings = {cid: discover_bindings()[cid] for cid in BATCH12_IDS}
    ...
    created.append(
        write_json(
            "BATCH12_CAPABILITY_INVENTORY_551_600.json",
    ...
    created.append(
        write_json(
            "BATCH12_INTERNAL_DUPLICATE_ANALYSIS.json",
            {
                "artifact": "BATCH12_INTERNAL_DUPLICATE_ANALYSIS",
    ...
    created.append(
        write_json(
            "BATCH12_CROSS_BATCH_DUPLICATE_ANALYSIS.json",
            {
                "artifact": "BATCH12_CROSS_BATCH_DUPLICATE_ANALYSIS",
                "generated_at_utc": now,
                "git_commit": head,
    ...
    created.append(
        write_json(
            "BATCH12_EXISTING_PARTIAL_REUSE_ANALYSIS.json",
    ...
    created.append(
        write_json(
            "BATCH12_CANONICAL_DECISIONS.json",
            {
                "artifact": "BATCH12_CANONICAL_DECISIONS",
                "generated_at_utc": now,
                "git_commit": head,
    ...
    created.append(write_json("BATCH12_RTM_551_600.json", build_rtm(bindings, catalog, head)).name)

    created.append(
        write_json(
            "BATCH12_SEMANTIC_EVIDENCE.json",
            {
                "artifact": "BATCH12_SEMANTIC_EVIDENCE",
                "generated_at_utc": now,
                "git_commit": head,
    ...
    created.append(
        write_json(
            "BATCH12_INDEPENDENT_ORACLE_EVIDENCE.json",
            {
                "artifact": "BATCH12_INDEPENDENT_ORACLE_EVIDENCE",
                "generated_at_utc": now,
                "git_commit": head,
    ...
    created.append(
        write_json(
            "BATCH12_CONSUMER_PATH_EVIDENCE.json",
            {
                "artifact": "BATCH12_CONSUMER_PATH_EVIDENCE",
                "generated_at_utc": now,
                "git_commit": head,
    ...
    created.append(
        write_json("BATCH12_DOMAIN_SPEC_APPLICABILITY.json", build_domain_spec_applicability(head)).name
    )
    created.append(
        write_json("BATCH12_TEMPORAL_INTELLIGENCE_APPLICABILITY.json", build_temporal_applicability(head)).name
    )
    created.append(write_json("BATCH12_SECURITY_MATERIAL_PATH_AUDIT.json", build_security_audit(head)).name)
    created.append(write_json("BATCH12_ENTITLEMENT_AUDIT.json", build_entitlement_audit(head)).name)

    created.append(
        write_json(
            "BATCH12_CROSS_BATCH_REGRESSION.json",
            {
                "artifact": "BATCH12_CROSS_BATCH_REGRESSION",
                "generated_at_utc": now,
                "git_commit": head,
                "suites": regression,
    ...
    created.append(
        write_json(
            "BATCH12_FORMAL_GATE_PROVENANCE.json",
            {
                "artifact": "BATCH12_FORMAL_GATE_PROVENANCE",
                "generated_at_utc": now,
                "final_code_sha": head,
    ...
    created.append(write_json("BATCH12_SSOT_RECONCILIATION.json", build_ssot_reconciliation(head)).name)
    ...
    flags = {
        "BATCH12_FINAL_LOCAL_FREEZE": freeze_ok,
        ...
        "CROSS_BATCH_PAIRS_REVIEWED_27500": exhaustive["evaluated_cross_batch_pairs"] >= EXPECTED_CROSS_BATCH_PAIRS,
        ...
        "CANONICAL_DECISION_551_TO_88": True,
    ...
    created.append(
        write_json(
            "BATCH12_FINAL_LOCAL_FREEZE.json",
            {
                "artifact": "BATCH12_FINAL_LOCAL_FREEZE",
                "generated_at_utc": now,
                "final_head": head,
                "branch": BRANCH,
                "capability_range": "551-600",
                "formal_gate_provenance": "docs/BATCH12_FORMAL_GATE_PROVENANCE.json",
                "semantic_evidence": "docs/BATCH12_SEMANTIC_EVIDENCE.json",
                "independent_oracle_evidence": "docs/BATCH12_INDEPENDENT_ORACLE_EVIDENCE.json",
                "consumer_paths": "docs/BATCH12_CONSUMER_PATH_EVIDENCE.json",
                "regression": "docs/BATCH12_CROSS_BATCH_REGRESSION.json",
                "ssot_reconciliation": "docs/BATCH12_SSOT_RECONCILIATION.json",
                "flags": flags,
                "preserved_statuses": {
