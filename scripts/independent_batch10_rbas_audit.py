#!/usr/bin/env python3
"""Independent Third-Line RBAS Due Diligence — Official Batch 10 (IDs 251–300).
Master Contract Run 29 — RBAS-001 risk-based scoping with Tier1 full 9-phase / Tier2 abbreviated."""
...
def batch10_tier_map():
    return batch_tier_map(10)


WF027_IN_BATCH10 = WF027_UNRESOLVED_LEGACY_IDS & set(range(451, 501))

OUT = ROOT / "institutional_due_diligence_2026" / "batch10_independent_audit"
OUT.mkdir(parents=True, exist_ok=True)

BATCH10_RANGE = range(451, 501)
...
BATCH10_SPINES = frozenset({"batch10_prep", "batch10"})
...
async def split_brain_test(cid: int) -> dict[str, Any]:
    ...
    from cap646.batch10_dedicated import BATCH10_DEDICATED_IDS, execute as execute_b6d
    ...
    if cid in BATCH10_DEDICATED_IDS:
        try:
            dedicated = await execute_b6d(cid, params=params)
            row["batch10_dedicated_available"] = True
            row["dedicated_result"] = dedicated
        except Exception as exc:
            row["batch10_dedicated_available"] = False
            row["dedicated_error"] = f"{type(exc).__name__}: {exc}"
    else:
        row["batch10_dedicated_available"] = False
        row["dedicated_error"] = "not_in_BATCH10_DEDICATED_IDS"
    ...
    elif not row.get("free_tier_available") and row.get("batch10_dedicated_available"):
        row["result_type"] = "DEDICATED_ONLY"
        row["verdict"] = "No free_tier path — dedicated batch10 only (SPLIT-BRAIN N/A)"
    elif row.get("free_tier_available") and row.get("batch10_dedicated_available"):
        f_data = (row.get("free_result") or {}).get("data") or row.get("free_result")
        d_raw = row.get("dedicated_result") or {}
        d_data = d_raw.get("data")
        if d_data is None:
            from cap646.batch10_dedicated import EXPECTED_SURFACE
...
async def load_split_brain() -> dict[int, dict]:
    ...
    if _split_cache is not None:
        return _split_cache
    rows = [await split_brain_test(cid) for cid in BATCH10_RANGE]
...
def backend_source(cid: int) -> tuple[str, list[str]]:
    from cap646.batch10_dedicated import BATCH10_DEDICATED_IDS

    mod = ROOT / "cap646" / "batch10_dedicated.py"
    ...
    if cid in BATCH10_DEDICATED_IDS:
        return "cap646.batch10_dedicated", lines_out or [f"_cap{cid:03d} handler"]
    return "cap646.batch10_production unmapped", [f"capability {cid} not in BATCH10_DEDICATED_IDS"]
...
def cross_spine_preflight(cid: int) -> tuple[str, str | None]:
    ...
    if cid in CROSS_SPINE_RESOLVED_IDS:
        return "PASS", "Run 29 cross-spine resolved — batch10 spine only"
...
def phase1_conceptual(cid: int, result: dict, *, split: dict) -> tuple[str, str | None]:
    ...
    if result.get("production_spine") not in BATCH10_SPINES:
        return "FAIL", f"CROSS_SPINE: official batch10 but production_spine={result.get('production_spine')}"
...
def phase7_iso(cid: int, result: dict) -> tuple[str, str | None]:
    ...
    if result.get("production_spine") not in BATCH10_SPINES:
        return "PARTIAL", f"traceability split: official batch10, spine={result.get('production_spine')}"
    return "PASS", None
...
def final_status_tier1(
    ...
    if spine not in BATCH10_SPINES:
...
async def audit_all() -> list[dict]:
    ...
    tier_map = batch10_tier_map()
    (OUT / "RBAS001_TIER_CLASSIFICATION.json").write_text(
        json.dumps([tier_map[cid] for cid in BATCH10_RANGE], indent=2, ensure_ascii=False) + "\n",
        encoding="utf-8",
    )
...
def _impact_metrics(rows: list[dict]) -> dict[str, Any]:
    ...
    return {
        ...
        "calibration_note": (
            "Escalation excludes compliance_footer provenance_score paths (Run 29 calibration). "
...
def write_report(rows: list[dict], tier_map: dict[int, dict[str, Any]]) -> Path:
    md = OUT / "BATCH10_INDEPENDENT_RBAS_AUDIT_REPORT.md"
    ...
    lines.extend(
        [
            ...
            f"- **Routing overlaps (BATCH01∩BATCH02∩BATCH03∩BATCH10):** "
            f"{len(overlaps)} IDs `{sorted(overlaps.keys())}`\n",
            f"- **WF-027 dormant legacy in batch10 range (251–300):** `{sorted(WF027_IN_BATCH10)}` "
            f"(zero overlap — unresolved legacy IDs 584+ are outside range)\n",
            "- **Cross-spine Run 29:** no WF-027 IDs in scope; batch10_prep spine registered for all 50\n\n",
            "## Results Table\n\n",
    ...
    for k, v in sb.most_common():
        lines.append(f"- **{k}:** {v}\n")
    lines.append("\n### WF-027 Preflight — Zero Overlap (Run 29)\n\n")
    lines.append(
    ...
    if not CONCEPTUAL_FLAGS:
        lines.append("- *(none flagged in Run 29 static pre-scan — live audit above is authoritative)*\n")
    lines.append("\n## رأي اللجنة المستقلة\n\n")
    lines.append(
        f"بصفتنا لجنة تدقيق مستقلة (Third Line of Defense — IIA IPPF)، وبعد تنفيذ RBAS-001 "
        f"على Batch 10 (IDs 251–300) وفق SR 26-2 وCOSO وGIPS وRTM-IND-001 وCROSS-SPINE-001، "
        ...
        f"**Batch 10 غير مغلق** — بوابة الإغلاق: CONCEPTUALLY-UNSOUND=0 + RTM صادق. "
        ...
        f"spine=batch10_prep. **نوصي بعدم أي إصلاح قبل مراجعة هذا التقرير** — "
        f"الإغلاق في Run منفصل كما Batch 01/02/03.\n"
    )
    md.write_text("".join(lines), encoding="utf-8")
    (OUT / "BATCH10_INDEPENDENT_RBAS_AUDIT.json").write_text(
