#!/usr/bin/env python3
"""Live production-path proof for Batch 02 (826-completion, IDs 101–150)."""
...
async def _prove_one(capability_id: int) -> dict:
    ...
    result = await execute_capability(
        ...
        params={
            ...
            "email": "batch02-proof@blackdark.local",
            "tier": "pro",
        },
    )
    ...
    spine = "batch01" if capability_id in BATCH02_OVERLAP_BATCH01 else "batch02"
    module = f"cap646.{spine}_production"
    entrypoint = (
        f"cap_{capability_id:03d}"
        if capability_id in BATCH02_OVERLAP_BATCH01
        else batch02_entrypoint(capability_id)
    )
    production_path = f"cap646.runtime.execute_capability -> {module}.{entrypoint}"
    ...
    return {
        ...
        "option_a_verified": (
            ...
            and surface_ok
            and live_surface not in GENERIC_SURFACES
        ),
    }


async def main() -> None:
    proofs = [await _prove_one(cid) for cid in sorted(BATCH02_IDS)]
    out = {
        "verified_at": datetime.now(UTC).isoformat(),
        "batch": "batch02_826_completion",
        "scope": "IDs 101–150 (50 capabilities)",
        "production_runtime_path": "api/routers/cap646.py -> cap978.unified -> cap646.runtime.execute_capability -> cap646.batch02_production",
        "capability_ids": sorted(BATCH02_IDS),
        "overlap_batch01_ids": sorted(BATCH02_OVERLAP_BATCH01),
        "all_verified": all(p["option_a_verified"] for p in proofs),
        "proofs": proofs,
    }
    json_path = ROOT / "docs/BATCH02_PRODUCTION_PROOF.json"
    ...
    if not out["all_verified"]:
        ...
        raise SystemExit(f"batch02 proof failed for: {failed}")
