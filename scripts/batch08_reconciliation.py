#!/usr/bin/env python3
"""Batch08 institutional reconciliation helpers — audit-defect closure only."""
...
REUSED_LINK_CATALOG: dict[int, dict[str, Any]] = {
    379: {
        ...
        "facade_binding": "bd_platform.arbitrage_portfolio_ux_layer.analyze_liquidity_capacity_189",
        "evidence": "docs/RETROSPECTIVE_DEEP_AUDIT_BATCH_04_301_400.json row capability_id=379 delegates to canonical #189",
        "mece_action": "Batch08 liquidity-capacity surface reuses canonical #189 — no parallel liquidity engine",
    },
}
...
HERO_FACADE_DELEGATIONS: dict[int, dict[str, Any]] = {
    382: {
        ...
        "underlying_module": "bd_platform.heroes_capability_layer",
        "underlying_function": "single_sentence_financial_button_382",
        "facade_binding": "bd_platform.heroes_capability_layer.single_sentence_financial_button_382",
        "evidence": "Distinct Single-Sentence Oracle CTA facade; contextual Hero surface only",
        "hero_impact": "NO_HERO_FEED — UI CTA facade; does not double-count Hero inputs",
    },
}

PERF_CLASS_MAP: dict[int, str] = {
    369: "CLASS_C_AI_HEAVY",
    379: "CLASS_C_AI_HEAVY",
    384: "CLASS_C_AI_HEAVY",
    385: "CLASS_C_AI_HEAVY",
    386: "CLASS_C_AI_HEAVY",
    387: "CLASS_C_AI_HEAVY",
    390: "CLASS_C_AI_HEAVY",
    392: "CLASS_C_AI_HEAVY",
    396: "NEWS_FEED",
    382: "CLASS_A_DIRECT_LIGHTWEIGHT",
}
...
# Institutional local performance methodology (Batch08 micro-closure)
...
INDEPENDENT_QUEUE = [
    {
        ...
        "prerequisite": "G0-G5 local complete; BATCH08_12207_VALIDATION_PACKAGE.json",
        "evidence_consumed": "docs/BATCH08_12207_VALIDATION_PACKAGE.json",
    ...
    },
    {
        ...
        "evidence_consumed": "docs/BATCH08_12207_TRANSITION_PACKAGE.json + OPERATION package",
    ...
    },
    {
        ...
        "pre_live_or_post_live": "PRE_LIVE",
        "prerequisite": "BATCH08_SRE_PRR_PACKAGE.json PRR_PREPARATION_COMPLETE_LOCAL",
        "evidence_consumed": "docs/BATCH08_SRE_PRR_PACKAGE.json",
    ...
    },
    {
        ...
        "pre_live_or_post_live": "PRE_LIVE",
        "prerequisite": "BATCH08_G7_PRE_ASSURANCE_PACKAGE.json G7_LOCAL_PREPARATION_COMPLETE",
        "evidence_consumed": "docs/BATCH08_G7_PRE_ASSURANCE_PACKAGE.json + evidence index",
...
RAILWAY_QUEUE = [
    ...
    {
        "queue_id": "RL2",
        "description": "Gate Zero live health + cap646 probes (351-400)",
    ...
    {
        "queue_id": "RL5",
        "description": "Per-ID PASS_LIVE elevation (351-400)",
...
def compute_drift_metrics(tested_head: str, final_head: str) -> dict[str, Any]:
    ...
    assurance_tooling = [
        ...
        and (
            "batch08" in f
            or f == "scripts/partial_batches/batch_08_351_400.json"
            or f == "scripts/reconcile_batch08_institutional.py"
        )
    ]
...
def build_layer_a_internal_pairwise(
    ...
    """Layer A: 351-400 vs 351-400 ONLY — no cross-batch REUSED-LINK counts."""
        ...
        per_id_rows.append(
            {
                ...
                "note": "Layer A scope is intra-Batch08 only; cross-batch reuse (#379→#189) is Layer B",
            }
        )
    ...
    return {
        "scope": "Layer A — Batch08 IDs 351-400 vs 351-400 ONLY",
...
def build_layer_b_cross_batch(
    ...
    for cid in BATCH08_IDS:
            ...
            rows.append(
                {
                    ...
                    "note": "Contributes via canonical #70 only — #379 facade does not add second Hero liquidity score",
                }
            )
            continue
        ...
        prior_matches = [
            ...
            if prior_id < 351 and prior_pair == target
        ]
        ...
        rows.append(
            {
                ...
                "evidence": f"pdf_capability_registry binding unique for batch08 #{cid}",
                "decision": decision,
    ...
    return {
        "scope": "Layer B — Batch08 IDs 351-400 vs prior capabilities 1-350",
        "summary": {
...
def build_existing_verified_evidence(
    ...
    for cid in BATCH08_IDS:
        ...
        row = {
            ...
            "evidence_locations": [
                "docs/docs/RETROSPECTIVE_DEEP_AUDIT_BATCH_04_301_400.json",
                "data/hero_batch_08_351_400_evidence.jsonl",
            ],
    ...
    return {
        "artifact": "BATCH08_EXISTING_VERIFIED_EVIDENCE",
        "generated_at": datetime.now(UTC).isoformat(),
        "git_commit": baseline_head,
        "scope": "Per-ID EXISTING_VERIFIED / CLOSED_REUSED_LINK evidence chain — Batch08 351-400",
        "summary": {
...
def build_collective_review_local(
    ...
    layer_b_by_id = {r["batch08_id"]: r for r in duplicate_doc["layer_b_cross_batch"]["per_id"]}
    rows = []
        ...
        rows.append(
            {
                ...
                "review_evidence": [
                    "docs/BATCH08_EXISTING_VERIFIED_EVIDENCE.json",
                    "docs/BATCH08_DUPLICATE_CANONICAL_ANALYSIS.json",
                    "docs/BATCH08_RTM_351_400.json",
                ],
                "review_evidence_location": f"docs/BATCH08_COLLECTIVE_REVIEW_LOCAL.json#capability_id={cid}",
                "binding_reviewed": f"{mod}.{fn}",
            }
        )

    return {
        "artifact": "BATCH08_COLLECTIVE_REVIEW_LOCAL",
        "generated_at": datetime.now(UTC).isoformat(),
        "git_commit": baseline_head,
...
async def _benchmark_capability(
    ...
    *,
    ...
    return {
        ...
        "evidence": f"docs/BATCH08_PERFORMANCE_CAPACITY_PREP.json#capability_id={cid}",
    }
...
async def run_local_performance_benchmark() -> dict[str, Any]:
    ...
    for cid in retry_ids:
        ...
        if retry["measurement_status"] != "LOCAL_MEASURED_INSUFFICIENT":
            by_id[cid] = retry
    results = [by_id[cid] for cid in BATCH08_IDS]
