...
> **تحديث v5:** مراجعة مستقلة نقطةً بنقطة لجميع أقسام v4، مع إعادة التحقق من حداثة وسلطة المراجع حتى 6 سبتمبر 2026، وتصحيح التصنيفات التي قد توحي خطأً بأن إرشادًا أو benchmark هو معيار ملزم. v5 يحتفظ بجوهر v4 لكنه يضيف سجل صلاحية للمراجع، وحدود الانطباق التنظيمي، ومصفوفة مراجعة 0–139، ويحدّث DORA إلى نموذج المقاييس الخمسة الحالي.
>
> **Delta Hardening (6 Sep 2026 — in-place):** تقوية delta فقط دون إعادة كتابة v5: سجل المتطلبات المعياري، فصل requirement/guidance/evidence/claim/decision/exception/residual risk، دورة حياة الضوابط الكاملة، عقود البيانات وPoint-in-Time Integrity، تصنيف intelligence/model risk tiers وChampion/Challenger، crosswalk أمني machine-verifiable، SLI/SLO/error budget، human-centred assurance، Committee Reperformance Pack، NO-GO hardening، وفصل BACKTESTED/SIMULATED/LIVE/INDEPENDENT. راجع §139 لسجل التحقق السباعي.
...
## 0.1 أنواع الالتزام والأدلة — Requirement vs Claim vs Exception

**[POLICY — Delta Hardening]** لمنع الخلط في اللجان والمراجعات، افصل صراحةً:

| النوع | المعنى | مثال |
|---|---|---|
| **Requirement** | التزام معياري أو سياسة مشروع يجب استيفاؤه | entitlement قبل handler |
| **Guidance** | إرشاد موثّق غير ملزم حرفيًا إلا إذا اعتُمد `[POLICY]` | Google SRE PRR checklist |
| **Evidence** | دليل قابل لإعادة التحقق على claim | test log + commit SHA |
| **Claim** | ادعاء حالة/نجاح/جاهزية | `PASS_ENGINEERING` |
| **Decision** | قرار canonical/duplicate/build/reuse | `KEEP_DISTINCT` |
| **Exception / Waiver / Risk Acceptance** | انحراف مخول مؤقتًا بسجل رسمي | Exception_ID + expiry |
| **Residual Risk** | خطر متبقٍ بعد controls/compensating controls | High tenant-abuse residual |

**قواعد:**
- Claim لا يُعادل Requirement.
- Evidence لا يُعادل Operating Effectiveness.
- Exception لا يلغي Requirement؛ يؤجل/يستبدل enforcement حتى expiry أو reassessment.
- Residual risk يجب أن يكون مرئيًا في closure/assurance pack وليس مخفيًا داخل aggregate score.
...
تُعتبر القدرة **COMPLETE_ENGINEERING** فقط إذا ثبت جميع ما يلي بحسب الانطباق:
...
لا تستخدم حالة **LIVE_READY** إلا إذا ثبت، في البيئة الحقيقية أو مسار نشر ممثل بدرجة كافية:

- النشر ناجح.
- مسار المستخدم الحقيقي يعمل.
- بيانات/اعتماديات الإنتاج المطلوبة متاحة.
- Authorization/Entitlements تعمل.
- SLI/SLO مقاسة.
- Logging/metrics/traces أو البديل المؤسسي متاح.
- خطط rollback/recovery متاحة.
- لا يوجد blocker خارجي غير محلول.
- نتائج smoke/E2E الحية ناجحة.
- لا يوجد اختلاف معروف يجعل الاختبار المحلي غير ممثل للإنتاج.

**ممنوع:** `tests passed` ⇒ `LIVE_READY`.
...
## 8.1 Normative Requirements Register — السجل المعياري للمتطلبات

**[POLICY — Delta Hardening]** **[STD-aligned]** مستلهم من ISO/IEC/IEEE 29148 traceability و§8 RTM؛ سجل واحد canonical يكفي — لا تُنشئ register موازٍ بلا سبب (§113.4).

لكل requirement مادي، سجل على الأقل:

`Requirement_ID → Source/Authority → Applicability → Materiality/Risk → Owner → Acceptance Criteria → Implementation/Evidence Expectation → State → Exceptions → Review/Expiry`

| الحقل | المعنى |
|---|---|
| **Requirement_ID** | معرف ثابت قابل للتتبع عبر RTM/controls |
| **Source/Authority** | `[STD]` / `[GUIDE]` / `[POLICY]` / `[SUPERVISORY BENCHMARK]` مع مرجع محدد |
| **Applicability** | capability/domain/tenant tier/environment |
| **Materiality/Risk** | M0–M4 أو risk tier |
| **Owner** | RACI owner (§82) |
| **Acceptance Criteria** | oracle/invariant قابل للاختبار — ليس "works" |
| **Implementation/Evidence Expectation** | canonical code/route + minimum evidence grade (§130) |
| **State** | NOT_STARTED / IN_BUILD / PASS_ENGINEERING / … (§104) |
| **Exceptions** | Exception_IDs المرتبطة أو `[]` |
| **Review/Expiry** | تاريخ مراجعة/انتهاء للاستثناء أو revalidation |

**قاعدة:** أي requirement بلا owner أو acceptance oracle مادي = `ASSURANCE_GAP` حتى الإكمال.
...
**[STD-aligned]** ISO/IEC 25012 + **[POLICY — Delta Hardening]** لكل source/dataset مادي، عقد بيانات formal يغطي على الأقل:
...
- **event time vs ingest time** (§14.4)
- freshness/timeliness threshold
...
- evidence and monitoring hooks

الحقول القديمة (schema, types, units, timezone, timestamps, missing behavior, precision, update frequency, freshness threshold, provenance, licensing/usage constraints) تبقى إلزامية حيث تنطبق؛ القائمة أعلاه توسّعها ولا تستبدلها.

**قاعدة:** unknown material data rights = NO-GO (§87).
...
## 15.1 POINT-IN-TIME INTEGRITY CONTRACT — عقد سلامة الزمن

**[POLICY — Delta Hardening]** **[VALIDATED_CONDITIONAL]** لأي historical replay/backtest/evaluation/regime analysis مادي:
...
- survivorship bias حيث ينطبق
- post-event enrichment leakage

**ممنوع:** تمثيل retrospective replay كـhistorical live prediction (§130.1).
...
## 16.1 تصنيف صريح
كل قدرة تحمل "AI" أو intelligence/model/rule behavior تُصنف:
...
- ML_MODEL.
- LLM/GENERATIVE.
- **AGENTIC_COMPONENT** (multi-step tool-using/autonomous workflow حيث ينطبق)
- HYBRID.

**قواعد Delta Hardening:**
- **ممنوع** تسمية deterministic formula/rule engine كـAI/ML.
- إذا كانت المنطق deterministic بالكامل، صنّفه كـformula/rule engine حتى لو UI يقول "AI".
- لا AI-washing.
...
## 16.8 Intelligence / Model Risk Tiers — **[POLICY — Delta Hardening]**

صنّف intelligence/model/rule risk tier بناءً على:
...
**Tier أعلى ⇒ evidence أقوى:** validation, benchmark coverage, explainability, monitoring, rollback, human review, independent challenge (§69, §131).

## 16.9 Champion / Challenger Promotion — **[POLICY — Delta Hardening]**

لا promotion من challenger إلى champion/production بدون evidence pack محدد مسبقًا:
- acceptance oracle
- representative sample كافٍ
- regime coverage
- non-regression proof
- degradation guardrails
- rollback trigger
- documented promotion decision + owner + timestamp

**NO-GO:** unvalidated high-impact model promotion (§87).
...
## 17.8 Security Crosswalk — Machine-Verifiable — **[POLICY — Delta Hardening]**

**[GUIDE-aligned]** OWASP Threat Modeling + NIST SSDF + §53؛ crosswalk واحد machine-verifiable (JSON/CSV/matrix) يغطي على الأقل:

`Threat/Risk → Control → Canonical Implementation → Verification → Evidence → Owner → Residual Risk`

**تقوية إلزامية حيث تنطبق:**
- threat modeling (§17.1)
- privileged access governance
- tenant isolation (§68)
- secrets lifecycle (§17.4, §72)
- vulnerability severity/remediation policy (§59)
- dependency/supply-chain admission (§56)
- SBOM/provenance (§56, §124.4)
- secure build/release evidence (§73, §124)
- incident escalation + evidence retention (§60, §127)

**قاعدة:** presence of control ≠ operating effectiveness (§129). Material controls require reproducible evidence.
...
## 20.6 Resilience Evidence Minimum — **[POLICY — Delta Hardening]**

**[GUIDE-aligned]** Google SRE + §79–§81؛ حيث ينطبق على journey/state مادي، اثبت:
- SLI/SLO/error budget linkage (§79)
- RTO/RPO/backup/restore verification (§80, §61, §72)
- failover validation أو documented gap + risk acceptance
- dependency criticality + graceful degradation (§20.1–§20.3)
- capacity thresholds (§63)
- recovery evidence location (immutable reference)

**NO-GO:** critical recovery path without evidence (§87).
...
## 22.1 Human-Centred Workflow Assurance — **[POLICY — Delta Hardening]**

**[STD-aligned]** ISO/IEC 25019 quality-in-use؛ لكل persona/workflow مادي:

`User → Goal → Context → Task → Expected Outcome → Effectiveness → Efficiency → Error/Recovery → Accessibility → Satisfaction → Evidence`

**قواعد:**
- Feature usage / route existence / `success=true` **لا** يثبت user value أو usability.
- Empty/error/loading/stale states جزء من user outcome evidence.
- WCAG 2.2 AA حيث ينطبق (§22).
...
أي انحراف عن معيار حاكم — **exception / waiver / risk acceptance** — يحتاج سجل رسمي:
...
Expiry Date:
Review Date:
Mandatory_Reassessment_Trigger:
Exit Plan:
Evidence:
```

### قواعد صارمة
- كل استثناء له **Expiry** و**Mandatory Reassessment** عند expiry أو trigger material change.
- لا استثناء دائم بلا مراجعة.
- لا يعتمد المنفذ استثناءه بنفسه في M3/M4.
- انتهاء الاستثناء يعيد الحالة تلقائيًا إلى NOT_ASSURED حتى التجديد أو العلاج.
- Exception ≠ Requirement removal؛ يؤجل enforcement أو يستبدله بcompensating control موثق.
- Residual risk بعد compensating controls يجب أن يكون explicit في closure/assurance pack.
...
`Control_ID → Risk/Threat → Requirement → Implementation → Test/Verification → Evidence → Owner → Frequency → Status → Residual Risk`

**Machine-verifiable crosswalk (§17.8):**
...
**Delta Hardening:** Material controls require reproducible evidence. Presence alone is never sufficient for operating effectiveness (§129).
...
**[GUIDE-aligned]** Google SRE SLI/SLO/error budgets؛ formalize chain:

`SLI → SLO → measurement window → target → error budget → burn/response policy → owner → alert`
...
- window.
- error budget.
- burn-rate response policy (freeze features / incident / rollback حسب سياسة المشروع).
- alert.
- owner.
...
**قاعدة:** aggregate green score لا يخفي SLO breach مادي (§87, §90).
...
**Material unresolved findings لا يجوز إخفاؤها بaggregate score** (§90).

- PASS بلا Evidence.
- Critical/High exploitable security issue غير معالج.
- **critical security defect** (exploitable or tenant/data breach path).
- unknown production artifact.
- no rollback for material irreversible change.
- cross-tenant leakage / **cross-tenant bypass**.
- silent data corruption.
- conflicting canonical calculations / **unresolved source-of-truth or formula conflict**.
- live claim without live-path evidence / **production claim supported only by backtest/simulation**.
- untested restore for critical state / **critical recovery path without evidence**.
- uncontrolled production secrets.
- model M3/M4 بلا owner/version/validation / **unvalidated high-impact model promotion**.
...
- **unknown material data rights** (§14.2).
- **point-in-time leakage** / look-ahead / future-information leakage (§15.1).
- **unsupported certification/compliance claim** (ISO/SOC/G6/G7/ASSURANCE_READY بلا proof).
- **material financial/user outcome not traceable** to data/formula/version/evidence.
...
## 104.1 قاموس حالة التنفيذ الحاكم
...
## 113.4 Single Canonical Register Rule — **[POLICY — Delta Hardening]**

- لا تُنشئ duplicate registers/documents عندما register canonical واحد يكفي (§8.1 Normative Requirements Register + RTM + Control Matrix).
- Documentation is evidence/supporting governance — **not** a substitute for working implementation.
- No arbitrary artifact name requirement overrides demonstrated technical outcome unless audit/reperformance/risk genuinely requires that artifact.
...
**Control lifecycle (Delta Hardening — canonical):**

`DESIGN → IMPLEMENTATION → OPERATING EFFECTIVENESS → LIVE EFFECTIVENESS → INDEPENDENT CHALLENGE`

افصل دائمًا:
- `CONTROL_DESIGN_EFFECTIVE`
- `CONTROL_IMPLEMENTED`
- `CONTROL_OPERATING_EFFECTIVE` (local/integrated reproducible evidence)
- `CONTROL_OPERATING_EFFECTIVE_LIVE` (production/live telemetry where required)
- `CONTROL_INDEPENDENTLY_CHALLENGED` (E5 / effective challenge)
...
إثباته في production/live telemetry = live operating evidence.
مراجعة مستقلة/effective challenge = independent assurance evidence.

**قواعد Delta Hardening:**
- Presence of a control **MUST NOT** be accepted as proof of operating effectiveness.
- Material controls require **reproducible evidence** at the appropriate lifecycle stage.
...
## 130.1 Evidence Environment Separation — **[POLICY — Delta Hardening]**

افصل صراحةً ولا تخلط labels:

| Label | المعنى |
|---|---|
| **BACKTESTED** | historical replay/evaluation under Point-in-Time contract |
| **SIMULATED** | synthetic/mocked/stubbed environment |
| **FORWARD LIVE/SHADOW** | forward observation in live/shadow without claiming historical prediction |
| **VERIFIED PRODUCTION** | deployed artifact + live-path evidence (E4) |
| **INDEPENDENT ASSURANCE** | effective challenge / independent review (E5) |

**قواعد:**
- BACKTESTED ≠ SIMULATED ≠ FORWARD LIVE/SHADOW ≠ VERIFIED PRODUCTION ≠ INDEPENDENT ASSURANCE.
- **ممنوع** تمثيل retrospective replay كـhistorical live prediction.
- Simulation/backtest success لا يUpgrade إلى PASS_LIVE أو production claim.

---

# 131. Committee Reperformance Standard — اللجنة تستطيع إعادة التحقق

**Committee Reperformance Pack (Delta Hardening)** — للـM2+، Evidence Pack يجب أن يسمح لمراجع مستقل بإعادة إثبات أي claim مادي **بدون developer memory**:
...
→ owner
→ canonical implementation
→ data/model/formula/config version
→ acceptance oracle
→ test
→ tested SHA/build
→ deployment identity (where applicable)
→ evidence (immutable reference)
→ exceptions/residual risk
→ local / integrated / live / independent state
```

خطوات reperformance:
1. تحديد requirement والـowner.
2. إيجاد canonical implementation.
3. معرفة source/data/model/formula/config version.
...
**Developer memory أو oral explanation MUST NOT be required** لإثبات claim مادي.
...
> **تاريخ المراجعة:** 6 سبتمبر 2026. تمت إعادة مراجعة جميع الأقسام الرئيسية 0–139 مقابل المصادر الرسمية/المطبقة والتمييز بين ما هو معيار ملزم، إرشاد، benchmark قطاعي، مواصفة صناعية، أو سياسة مشروع.
...
| 0.1 | أنواع الالتزام والأدلة — Requirement vs Claim vs Exception | STRENGTHENED_V5_DELTA |
| 8.1 | Normative Requirements Register | STRENGTHENED_V5_DELTA |
| 15.1 | POINT-IN-TIME INTEGRITY CONTRACT | STRENGTHENED_V5_DELTA |
| 16.8 | Intelligence / Model Risk Tiers | STRENGTHENED_V5_DELTA |
| 16.9 | Champion / Challenger Promotion | STRENGTHENED_V5_DELTA |
| 17.8 | Security Crosswalk — Machine-Verifiable | STRENGTHENED_V5_DELTA |
| 20.6 | Resilience Evidence Minimum | STRENGTHENED_V5_DELTA |
| 22.1 | Human-Centred Workflow Assurance | STRENGTHENED_V5_DELTA |
| 113.4 | Single Canonical Register Rule | STRENGTHENED_V5_DELTA |
| 130.1 | Evidence Environment Separation | STRENGTHENED_V5_DELTA |
| 139 | Delta Hardening Layer + Seven-Pass Validation Record | STRENGTHENED_V5_DELTA |
...
# 139. Delta Hardening Layer — September 2026 Research Refresh

> **Nature:** in-place delta hardening on v5 — **not** v6 rewrite.  
> **Authority classes used:** `[STD]` ISO/IEC 25012, 25019, 29148; `[GUIDE]` Google SRE, OWASP, NIST SSDF/CSF/800-61, NIST AI RMF; `[POLICY]` BLACKDARK project controls; `[SUPERVISORY BENCHMARK]` BCBS 239 / SR 26-2 **conditional only** — never compliance claim.

## 139.1 Delta Map — ما أُضيف أو قُوّي

| Domain | Canonical section | Delta action |
|---|---|---|
| A Governance / Requirements | §0.1, §8.1, §50 | Normative Requirements Register; requirement/guidance/evidence/claim/decision/exception/residual risk separation; formal exception/waiver/risk acceptance |
| B Control Assurance | §53, §129 | Full control lifecycle DESIGN→…→INDEPENDENT CHALLENGE; reproducible evidence; no presence=operating |
| C Data Quality & Contracts | §14.2, §15.1 | Expanded data contract fields; Point-in-Time Integrity Contract |
| D Intelligence / Model | §16.1, §16.8, §16.9, §69 | Deterministic vs AI classification; risk tiers; Champion/Challenger promotion evidence |
| E Security / Privacy / Supply Chain | §17.8, §53 | Machine-verifiable Threat→Control crosswalk; privileged access, SBOM, incident retention |
| F Reliability / Resilience | §79, §20.6, §80 | SLI→SLO→error budget chain; recovery evidence minimum |
| G Human-Centred Assurance | §22.1 | User→Goal→…→Evidence workflow chain; usage≠value |
| H Committee Reperformance | §131 | Full reperformance pack without developer memory |
| I NO-GO / Red Flags | §87 | Aggregate score cannot hide material unresolved findings |
| J Evidence Integrity | §130.1 | BACKTESTED/SIMULATED/LIVE/PRODUCTION/INDEPENDENT separation |
| K Reference Discipline | §0, §117, §136, §137 | Authoritative/current sources only; draft downgraded; practice≠standard |
| L Anti-Bureaucracy | §113.4 | Single canonical register; docs≠implementation substitute |

## 139.2 Authoritative Sources Used (Delta)

- ISO/IEC 25012:2008 — data quality model **[STD]**
- ISO/IEC 25019:2023 — quality-in-use **[STD]**
- ISO/IEC/IEEE 29148:2018 — requirements traceability **[STD]**
- ISO/IEC 42001:2023 / ISO/IEC 23894:2023 — AI governance **[STD]**
- NIST SP 800-218 SSDF v1.1 — secure SDLC **[GUIDE]**
- NIST CSF 2.0 / NIST AI RMF 1.0 — cyber/AI risk **[GUIDE]**
- OWASP ASVS 5.0 / Threat Modeling — application security **[GUIDE]**
- Google SRE — SLI/SLO/error budgets, PRR **[GUIDE — PRACTICE not STD]**
- AICPA TSC — control assurance semantics **[ASSURANCE CRITERIA]**
- BCBS 239 / Federal Reserve SR 26-2 — **conditional financial-grade benchmark only**; not BLACKDARK compliance claim

## 139.3 Unsupported / Draft Sources — Rejected or Downgraded

- ISO/IEC/IEEE 29148 Edition 3 DIS — **future-watch only** (§117)
- NIST SSDF v1.2 draft — **not governing** until final (§1.9)
- NIST AI RMF revision activity — monitor; do not claim next version compliance
- BCBS 239 / SR 26-2 — **not** direct regulatory compliance for BLACKDARK unless legal scope applies
- Google SRE — **never** labeled ISO standard
- Draft W3C/ISO replacements — not used until published

## 139.4 Seven-Pass Validation Record (6 Sep 2026)

| Pass | Result | Notes |
|---|---|---|
| **1 Source preservation** | PASS | No existing valid v5 requirement deleted or weakened; all prior §0–138 retained |
| **2 Semantic deduplication** | PASS | Delta extends canonical sections; §8.1/§53/§129 merged not duplicated |
| **3 Standards/source verification** | PASS | New normative statements tagged `[POLICY]` or aligned `[STD]`/`[GUIDE]`/`[ASSURANCE CRITERIA]` |
| **4 Cross-section consistency** | PASS | States (§104), gates G0–G7 (§47), RTM (§8), evidence ladder (§130), closure (§114) consistent |
| **5 Committee/audit sufficiency** | PASS | §8.1 + §131 enable full reperformance without developer memory |
| **6 Anti-false-claim review** | PASS | No unsupported certification/compliance/live/independent-assurance wording added |
| **7 Final integrity** | PASS | Structure readable; no malformed sections; no known internal contradiction |

`unresolved_standard_deficiencies=[]`

`V5_GOVERNING_STANDARD_UPDATED=true`

---

# 138. قرار v5 النهائي

**v5 — بعد Delta Hardening 6 Sep 2026 — هو المرجع الحاكم بعد المراجعة العميقة لجميع محتويات v4 وتقوية delta §139.**
