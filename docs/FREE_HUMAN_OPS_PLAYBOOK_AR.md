...
## إغلاق المسار المجاني المحلي (صارم)

| بند | حالة |
|-----|------|
| F1–F9 + F2 + F3 (أسرار، TOTP، تشغيل محلي، 60s، إضافة، Glass Box drafts، audit) | **منجز بشريًا** |
| Option A تسعير (Signup يختار خطة + ترقية تالية) | **مُدمَج على `main`** |
| Bootstrap بدون pyotp / Notepad غير فاضي | **مُدمَج على `main`** |
| Render Free public URL | **مؤجّل** — المنصة طلبت بطاقة تحقق (انظر `RENDER_FREE_AR.md`) |
| UptimeRobot على URL عام | **معلّق** حتى يوجد لينك عام بدون بطاقة أو بقرار واعٍ |
| نشر Glass Box على X/Telegram | بشري اختياري (حسابات مجانية) |

## بعد مجانية اليوم

انشر على أي هوست **مجاني بلا بطاقة** إن توفر → عيّن `APP_BASE_URL` → UptimeRobot مجاني → انشر مسودة Glass Box على حساب اجتماعي مجاني.

**قاعدة:** لا تدفع شيء ولا تُدخل بطاقة «للتحقق» حتى يثبت شخص واحد عادة Act/Wait — إلا بقرار مؤسس صريح خارج FREE playbook.
