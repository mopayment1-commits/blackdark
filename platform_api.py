...
@router.get("/intelligence-ledger/gate/status")
async def intelligence_ledger_gate_status_route():
...
@router.post("/intelligence-ledger/gate/evaluate")
async def intelligence_ledger_gate_evaluate_route(data: dict = Body(default={})):
...
@router.get("/intelligence-ledger/gate/hit-rate")
async def intelligence_ledger_gate_hit_rate_route(_admin: dict = Depends(require_admin)):
    from bd_platform.epistemic_humility_gate import get_gate_hit_rate_panel

    return get_gate_hit_rate_panel()


@router.get("/intelligence-ledger/gate/e2e")
async def intelligence_ledger_gate_e2e_route(_admin: dict = Depends(require_admin)):
