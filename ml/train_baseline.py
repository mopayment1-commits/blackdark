...
Trains a direction classifier from labeled oracle predictions + feature vectors.
Uses scikit-learn GradientBoosting (lightweight, no GPU required).
"""
...
def _build_training_frame(rows: list[dict[str, Any]]):
    ...
    for row in rows:
        ...
        for col in FEATURE_COLUMNS:
            if col in {"opportunity_score", "confidence"}:
                continue
...
async def train_oracle_direction_model(*, min_samples: int | None = None) -> dict[str, Any]:
    ...
    logger.info(
        "Baseline model trained | version=%s accuracy=%.3f samples=%d",
        version,
        accuracy,
        len(frame),
    )
...
async def model_status() -> dict[str, Any]:
    ...
    labeled = await fetch_labeled_oracle_predictions(limit=10000)
