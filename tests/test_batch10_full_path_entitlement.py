"""Batch10 full-path local integration — cap646 runtime with entitlement checks."""
...
PRO_USER = {"id": 1, "email": "batch10-fullpath@blackdark.local", "tier": "pro", "role": "user"}
...
async def test_full_path_allow_via_cap646_runtime(capability_id: int):
    ...
    if capability_id == 458:
        assert result.get("capability_id") in {458, 86}
    elif result.get("requested_capability_id") == capability_id:
...
def test_batch10_entitlement_requirement_matrix():
    ...
    for cid in _batch_ids():
        ...
        if _ORG_PERMISSIONS.get(cid) or row.get("feature_key") or row.get("usage_meter_key"):
            restricted.append(cid)
    assert restricted == [], f"unexpected restricted Batch10 IDs: {restricted}"
