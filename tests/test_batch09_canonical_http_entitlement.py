"""Batch09 canonical HTTP entitlement path — real TestClient, no entitlement mocks."""
...
def test_http_get_real_path_pro_user_allowed(client: TestClient, capability_id: int):
    ...
    async def _pro_user() -> dict:
        return {"id": 1, "email": "batch09-http@blackdark.local", "tier": "pro", "role": "user"}
...
def test_batch09_entitlement_requirement_matrix():
    ...
    for cid in _batch_ids():
        ...
        if _ORG_PERMISSIONS.get(cid) or row.get("feature_key") or row.get("usage_meter_key"):
            restricted.append(cid)
    assert restricted == [], f"unexpected restricted Batch09 IDs: {restricted}"
